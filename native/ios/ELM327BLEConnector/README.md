...
- Capability-gated follow-up reads only for PIDs and Mode 09 information types reported by the vehicle. PID support starts at page `00` and requests continuation pages through `E0` only when each continuation PID is reported. Header-bearing responses retain the responding ECU scope for supported PID and live PID values instead of flattening multiple ECU replies.
