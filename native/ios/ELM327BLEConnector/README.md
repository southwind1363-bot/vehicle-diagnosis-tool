...
- One fixed read-only setup and core queue: adapter identity/protocol, stored/pending/permanent DTCs, raw Mode 06 monitor tests, Mode 01 PID support/readiness/live values, Mode 02 freeze-frame support and supported values, and scoped Mode 09 calibration-ID, calibration-verification-number, and ECU-name support. Mode 09 follow-up requests are sent only when the ECU advertises the corresponding information type. VIN is not requested. The live set covers load, bank-1 fuel trims, MAP, RPM, vehicle speed, timing advance, coolant and intake temperatures, MAF, throttle position, and control-module voltage only when each PID is reported supported.
- Mode 06 output preserves only TID, CID, value, minimum, maximum, and responding ECU scope. TID/CID naming and units remain vehicle-specific and are not inferred.
- PID 01 readiness archives include the status bytes and their explicit monitor states. Spark and compression layouts remain separate, and unadvertised monitors are retained as not supported.
- Capability-gated follow-up reads only for PIDs and Mode 09 information types reported by the vehicle. PID support starts at page `00` and requests continuation pages through `A0` only when each continuation PID is reported. Header-bearing responses retain the responding ECU scope for supported PID and live PID values instead of flattening multiple ECU replies.
