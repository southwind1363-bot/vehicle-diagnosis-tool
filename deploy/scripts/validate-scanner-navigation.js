...
import "./validate-static-data-fetch.js";
import "./validate-scanner-import-cancel.js";

const source = fs.readFileSync(new URL("../script.js", import.meta.url), "utf8");
const html = fs.readFileSync(new URL("../index.html", import.meta.url), "utf8");
const css = fs.readFileSync(new URL("../style.css", import.meta.url), "utf8");
const timestampSource = source.match(/function formatObdTimelineTimestamp\(value\) \{[\s\S]*?\r?\n\}/)?.[0];
assert.ok(timestampSource);
const timestamp = vm.runInNewContext(`${timestampSource}; formatObdTimelineTimestamp`, { NO_DATA: "未取得" });
assert.equal(timestamp(null), "未取得");
assert.equal(timestamp("invalid-time"), "invalid-time");
assert.match(timestamp("2026-07-17T00:00:05.125Z"), /:05\.125/);
assert.notEqual(timestamp("2026-07-17T00:00:00Z"), timestamp("2026-07-17T00:00:05Z"));
