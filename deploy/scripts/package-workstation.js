...
const RUNTIME_FILES = ["start-workstation.cmd", "verify-workstation.cmd", "inspect-workstation-j2534.cmd", "scripts/inspect-workstation-j2534.js", "scripts/verify-workstation-package.js", "scripts/start-local-workstation.js", "scripts/workstation-assets.js", "scripts/j2534-readonly-worker.js", "scripts/j2534-dtc-selection-handoff.js", "scripts/j2534-native-quarantine.js", "scripts/j2534-registered-driver-native-preflight.js", "scripts/j2534-uds-readout-attempt-controller.js", "scripts/j2534-uds-transport-adapter-request.js", "scripts/j2534-uds-preparation-evidence.js"];
...
export function packageWorkstation(options = {}) {
  ...
  try {
    ...
    fs.appendFileSync(path.join(staging, "README.txt"), [
      "", "更新前のバックアップと復帰手順",
      "車両読取中には更新しません。読取を停止し、通常の手順で接続を終了してから保存・更新してください。",
      "整備事例はデータ管理の「JSONバックアップ」、診断の読取結果は「読取結果をJSON保存」で別々に保存します。保存先とファイルの存在を確認してください。",
      "旧版フォルダーとバックアップを残し、新版は別フォルダーへ全展開してください。旧版への上書きやファイル単位の混在は避けます。",
      "新版のverify-workstation.cmdで検査成功を確認後、旧版の起動画面でqとEnterを入力して終了し、新版のstart-workstation.cmdを開きます。",
      "同じブラウザープロファイルと同じURL（ホスト名・ポート番号を含む）を使用してください。localhostと127.0.0.1、異なるポート、公開サイトとローカル版は別の保存先です。",
      "初回再読込では保存済みの旧版画面が出る場合があります。オフライン更新の準備完了を確認し、未保存の作業がない状態で再読込してください。",
      `この配布物の版は${status.version}です。表示版と保存済み事例の件数・内容を確認してから利用してください。`,
      "移行先では整備事例を「JSONインポート」、読取結果を「保存した読取結果を開く」で別々に読み込み、内容を確認します。バックアップ原本は残してください。",
      "更新失敗や版不一致が続く場合は作業を止め、サイトデータやキャッシュを削除しないでください。配布物を一式移し直して検査するか、検査済みの旧版を同じURLで起動し直してください。",
      "旧版へ戻す前に新版をqとEnterで終了します。旧版への復帰では「更新版」通知が出るとは限りません。画面とオフライン基盤の版不一致案内を確認し、保存後に再読込して版とデータを確認してください。",
      "任意の旧版との保存形式互換性は保証しません。エラーやデータ欠落が続く場合は元の配布物とバックアップを残し、認証情報・接続キーを含めず管理者へ相談してください。",
      "ソフト検査の合格は、別PC・OS再起動・電源断・ディスク満杯・実VCI・実車適合の確認を意味しません。車両送信権限、消去操作、自動接続・自動読取・自動再試行はこの案内では有効にしません。", ""
    ].join("\n"));
