  ...
  try {
    ...
    if (mode01Archive) {
      ...
      const check = async file => {
        ...
        assert.deepEqual(await page.evaluate(() => obdDevSession.lastSession.supportedPidMatrix), expectedSupport);
        ...
        assert.match(await page.locator('.obd-simple-result-item').filter({ hasText: '対応PID' }).innerText(), /取得済み/);
      ...
      assert.deepEqual(JSON.parse(JSON.stringify(exported.supportedPidMatrix)), expectedSupport);
      await page.reload();
      await page.getByText('登録済み整備データを読み込みました。', { exact: false }).waitFor();
      await page.getByRole('button', { name: '7. OBD2車両読取', exact: true }).click();
      await check(saved);
    ...
    if (process.argv.includes('--development-source')) {
      ...
      const checkSource = async (file, fromResults = false) => {
        ...
        await page.getByRole('button', { name: '基本読取結果へ戻る', exact: true }).click();
        const note = page.locator('#obdSimpleResultNote');
        assert.equal(await note.isVisible(), true);
        assert.match(await note.innerText(), /^J2534開発検証データ（実車読取ではありません）。/);
        await note.screenshot({ path: path.join(output, 'development-source-normal.png') });
      };
      await checkSource(input);
      await require('./validate-readout-print.cjs')(page, output, 'development-overview');
      await page.locator('.obd-simple-result-item').first().click();
      await require('./validate-readout-print.cjs')(page, output, 'development-detail');
      await page.getByRole('button', { name: '基本読取結果へ戻る', exact: true }).click();
      ...
      await require('./validate-readout-print.cjs')(page, output, 'ordinary-replacement');
    ...
    for (const width of [1280, 390]) {
      ...
      for (const id of ['obdVehicleMaker', 'obdVehicleModel', 'obdVehicleModelCode', 'obdVehicleYear', 'obdVehicleEngineCode']) {
        ...
        const value = registeredVehicle[id] || await field.locator('option').evaluateAll(options => options.find(option => option.value && option.value !== '__manual__')?.value);
        if (!value) continue;
      ...
      assert.equal(selected.size, 5, 'Registered vehicle must exercise maker, model, model code, year and engine');
    ...
    const showTimeline = async () => {
      ...
      const previous = chart.getByRole('button', { name: /の前の記録$/ });
      const next = chart.getByRole('button', { name: /の次の記録$/ });
      ...
      assert.equal(await slider.inputValue(), String(expectedPoints - 2));
      await next.click();
      assert.equal(await slider.inputValue(), String(expectedPoints - 1));
      ...
      assert.equal(await slider.inputValue(), '0', 'Previous at start must not wrap');
      await next.click();
      await previous.click();
      assert.equal(await previous.evaluate(node => node === document.activeElement), true, 'Reaching the boundary must retain keyboard focus');
      ...
      for (const width of [320, 768]) {
        ...
        for (const dark of [false, true]) {
          ...
          if (clickedIndex < expectedPoints - 1) {
            await next.click();
            assert.equal(await slider.inputValue(), String(clickedIndex + 1));
            await previous.click();
      ...
      await next.click();
      assert.match(await playback.getAttribute('aria-label'), /記録を再生$/);
      const steppedPosition = await slider.inputValue();
      await page.waitForTimeout(150);
      assert.equal(await slider.inputValue(), steppedPosition, 'Step control must pause replay');
      await playback.click();
    ...
    for (const [caseIndex, readoutCase] of (restart ? [] : readoutCases).entries()) {
      ...
      const checkLiveDisplay = async () => {
        ...
        if (noLiveResponse) {
          ...
          const liveText = await page.locator('#obdMonitorGrid').innerText();
          assert.ok(liveText.includes(String(readoutCase.rpm)));
          if (caseIndex > 0) {
            assert.equal(liveText.includes('1726'), false, 'Recovery must show the new RPM, not the first scan');
            assert.doesNotMatch(await page.locator('#obdMonitorStatus').innerText(), /計測値は0項目です|表示できるライブ値がありません/);
            await page.screenshot({ path: path.join(output, 'synthetic-live-recovered.png') });
          }
        }
        await page.getByRole('button', { name: '基本読取結果へ戻る', exact: true }).click();
      };
      ...
      await page.screenshot({ path: path.join(output, `synthetic-${readoutCase.name}-disconnected.png`) });
      await page.reload();
      await page.getByText('登録済み整備データを読み込みました。', { exact: false }).waitFor();
      await page.getByRole('button', { name: '7. OBD2車両読取', exact: true }).click();
