...
check(details.includes("buildObdSupportedPidDisplayLines(supportedPidMatrix, supportedPidRecordIndexes)") && details.includes('sections.push(["対応PID", supportedPidLines])'), "PID display helper not wired");
const recordIndexes = [], indexedLines = context.buildObdSupportedPidDisplayLines(session.supportedPidMatrix, recordIndexes);
check(recordIndexes.length > 0 && recordIndexes.every((index) => /\] (対応PID|記録PID)/.test(indexedLines[index])), "Search includes state/error metadata");
