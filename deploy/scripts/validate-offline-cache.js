...
let waitingChanged;
const waitingWorker = { ...active, state: "installed", addEventListener: (_name, callback) => { waitingChanged = callback; } };
const waitingRegistration = { active: oldWorker, waiting: waitingWorker, update: async () => {}, addEventListener: () => {} };
const waitingClient = createStatusClient(statusWorker, waitingRegistration);
await waitingClient.registerOfflineCache();
check(waitingClient.offlineCacheStatus.textContent.includes("切替待ち"), "Installed waiting worker still displayed generic preparation");
check(waitingClient.offlineCacheStatus.textContent.includes("結果を保存") && !waitingClient.offlineCacheStatus.textContent.includes("準備済み"), "Waiting worker claimed readiness or omitted result preservation");
waitingRegistration.waiting = null;
waitingRegistration.active = waitingWorker;
waitingWorker.state = "activated";
waitingChanged();
await waitForStatusProbe(() => waitingClient.offlineCacheStatus.textContent.includes("準備済み"));
check(!waitingClient.offlineCacheStatus.textContent.includes("切替待ち"), "Waiting notice remained after verified activation");
