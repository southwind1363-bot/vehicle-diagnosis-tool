...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1079+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('const APP_VERSION = "2.438.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-13";'), "OBD app version should advance for primary review target direction ids");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1079");
