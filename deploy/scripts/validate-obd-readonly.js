...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1205+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('const primaryReviewTarget = summary.primaryReviewTargetReadoutId || summary.reviewActionSummary?.primaryReadoutId || "";') && appSource.includes('formatCoreReadoutLabel(primaryReviewTarget, primaryReviewTarget)'), "OBD app should format imported readout quality review targets");
check(appSource.includes('function formatReadoutQualityReviewRequestSummary(summary, fallback = NO_DATA)') && appSource.includes('primaryReadoutQualityReviewRequest') && appSource.includes('summary.primaryRequest') && appSource.includes('readoutQualityReviewRequestPlanSummary?.requestCount') && appSource.includes('read-only'), "OBD app should format readout quality review request summaries");
check(appSource.includes('const readoutQualityComparisonLabel = formatReadoutQualityComparisonSummary(session.importedReadoutQualityComparisonSummary, NO_DATA);') && appSource.includes('addObdDiagnosticFlowMetric(grid, "品質比較", readoutQualityComparisonLabel'), "OBD diagnostic flow panel should show imported readout quality comparisons");
...
check(appSource.includes('["品質比較", readoutQualityComparisonLabel]') && appSource.includes('const readoutQualityComparisonNote = formatReadoutQualityComparisonSummary(summarySource.importedReadoutQualityComparisonSummary, "");'), "OBD session summary and notes should expose imported readout quality comparisons");
...
check(appSource.includes('const APP_VERSION = "2.465.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-13";'), "OBD app version should advance for UI quality request plan aliases");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1205");
