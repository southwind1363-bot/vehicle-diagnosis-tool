...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(source.includes('function buildImportedSessionComparisonSummary({') && source.includes('coreReadoutInventoryComparison = null') && source.includes('core_readout_inventory_summary'), "obd-readonly should expose imported session comparison summaries");
...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1155+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('const APP_VERSION = "2.452.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-13";'), "OBD app version should advance for readout quality import/export carry-through");
...
check(scanSessionBridgeDiagnosticImportAlias.importedSessionComparisonSummary?.comparedSectionCount >= 6 && scanSessionBridgeDiagnosticImportAlias.importedSessionComparisonSummary?.sectionSummaryById?.core_readout_inventory_summary, "Diagnostic scan session did not include core readout inventory in imported session comparison count");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1155");
