...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(functionBody.includes('const readoutRequestPlanGateActionQueueByReasonId = Object.fromEntries(readoutRequestPlanGateActionQueue.map((item) => [item.reasonId, { ...item }]));') && functionBody.includes('actionQueueByReasonId: readoutRequestPlanGateActionQueueByReasonId,'), "readout request plan gate should index actions by blocked reason id");
    check(functionBody.includes('const readoutRequestPlanGateActionQueueByReadoutId = Object.fromEntries(readoutRequestPlanGateActionQueue.flatMap') && functionBody.includes('actionQueueByReadoutId: readoutRequestPlanGateActionQueueByReadoutId,'), "readout request plan gate should index actions by blocked readout id");
    ...
    check(functionBody.includes('actionSummary: readoutRequestPlanGateActionSummary,') && functionBody.includes('vehicleCommandEnabled: false'), "readout request plan gate action summary should stay read-only");
...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1491+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('const APP_VERSION = "2.522.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-14";'), "OBD app version should advance for readout request plan safety aliases");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1491");
