...
check(appSource.includes('const pendingReadoutRequestCount = Number.isFinite(Number(flow.pendingReadoutRequestCount))') && appSource.includes('` / queue ${pendingReadoutRequestCount}${mappedReadoutRequestCount !== null ? ` / mapped ${mappedReadoutRequestCount}` : ""}`'), "OBD diagnostic flow panel should show pending readout request queue count");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 662");
