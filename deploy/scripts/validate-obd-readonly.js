...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(source.includes('actionRequiredChanged: (importedGateSummary.actionRequired === true) !== (currentSummary.actionRequired === true)') && source.includes('nextActionChanged: (importedGateSummary.nextActionId || null) !== (currentSummary.nextActionId || null)'), "imported readout request plan gate comparison should compare next action state");
    ...
    check(source.includes('comparison.actionReadoutIdsChanged === true') && source.includes('request_plan_actions'), "imported session comparison should classify action readout changes as request plan action changes");
    ...
    check(source.includes('item.comparison.actionRequiredChanged === true') && source.includes('item.comparison.actionReadoutIdsChanged === true') && source.includes('Number(item.comparison.actionQueueCountDelta || 0) !== 0'), "imported session section summaries should treat request plan action changes as gate changes");
    check(source.includes('item.actionRequiredChanged === true || item.nextActionChanged === true || item.actionIdsChanged === true') && source.includes('item.actionReadoutIdsChanged === true') && source.includes('Number(item.actionQueueCountDelta || 0) !== 0'), "imported session top-level summaries should treat request plan action changes as gate changes");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 780");
