...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(source.includes('const primaryChangedIdDirection = changedIdSummaries.length > 0') && source.includes('primaryChangedIdDirectionSummary: primaryChangedIdDirection ? changedIdDirectionSummary[primaryChangedIdDirection] || null : null,'), "imported session comparison summaries should expose primary changed id direction");
...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 863+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 863");
