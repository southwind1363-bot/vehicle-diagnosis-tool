...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1187+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('["品質確認要求", readoutQualityReviewRequestLabel]') && appSource.includes('const readoutQualityReviewRequestNote = formatReadoutQualityReviewRequestSummary(summarySource.importedSessionComparisonSummary, "");'), "OBD session summary and notes should expose readout quality review requests");
...
check(appSource.includes('const APP_VERSION = "2.458.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-13";'), "OBD app version should advance for readout quality request plan summaries");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1187");
