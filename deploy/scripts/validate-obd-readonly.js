...
const mergeDiagnosticInputsFunctionChecks = () => {
  ...
  if (mergeDiagnosticInputsFunctionSource) {
    ...
    check(functionBody.includes('const importedReadoutQualityReviewRequestPlanSummary = importedSessionComparisonSummary?.readoutQualityReviewRequestPlanSummary || null;') && functionBody.includes('importedReadoutQualityReviewRequestPlanSummary,'), "mergeDiagnosticInputs should expose imported readout quality review request plan summaries");
...
const diagnosticScanSessionFunctionChecks = () => {
  ...
  if (diagnosticScanSessionFunctionSource) {
    ...
    check(functionBody.includes('const importedReadoutQualityReviewRequestPlanSummary = importedSessionComparisonSummary?.readoutQualityReviewRequestPlanSummary || null;') && functionBody.includes('importedReadoutQualityReviewRequestPlanSummary,'), "buildDiagnosticScanSession should expose imported readout quality review request plan summaries");
    check(functionBody.includes('const importedReadoutRequestPlanGateComparisonSummary = buildImportedReadoutRequestPlanGateComparisonSummary(importedReadoutRequestPlanGateSummary, readoutRequestPlanGateSummary);') && functionBody.includes('importedReadoutRequestPlanGateComparisonSummary,'), "buildDiagnosticScanSession should compare imported and recalculated readout request plan gate summary");
...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1191+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('const APP_VERSION = "2.459.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-13";'), "OBD app version should advance for top-level readout quality request plan summaries");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1191");
