...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1203+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('const readoutQualityReviewRequestLabel = formatReadoutQualityReviewRequestSummary(session.importedReadoutQualityReviewRequestPlanSummary || session.importedSessionComparisonSummary, NO_DATA);') && appSource.includes('addObdDiagnosticFlowMetric(grid, "品質確認要求", readoutQualityReviewRequestLabel'), "OBD diagnostic flow panel should show readout quality review requests");
...
check(appSource.includes('const APP_VERSION = "2.464.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-13";'), "OBD app version should advance for scan session quality request plan aliases");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1203");
