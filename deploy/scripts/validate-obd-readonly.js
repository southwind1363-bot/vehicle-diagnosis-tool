...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(source.includes('function buildReadoutRequestPlanSafetySummary(entries = [], unmappedRequestIds = [])') && source.includes('safeForBridgePlanning: allReadOnly && allNonTransmitting && unmappedCount === 0'), "obd-readonly should summarize readout request plan safety");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 678");
