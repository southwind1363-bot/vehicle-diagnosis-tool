...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(source.includes('const hasComparisonMetricChanges = (comparison = {}) => Number(comparison.completionDelta') && source.includes('comparison.emptyCountDelta') && source.includes('completionChanged: comparisons.some((item) => hasComparisonMetricChanges(item))'), "imported session comparison summaries should use one metric-change helper for section and top-level completion flags");
...
check(indexHtml.includes('id="obdDiagnosticFlowPanel"'), "OBD diagnostic flow panel container is missing from index.html");
check(appSource.includes('function renderObdDiagnosticFlowPanel(session = null)') && appSource.includes('renderObdDiagnosticFlowPanel(session);'), "OBD diagnostic flow panel renderer is missing from script.js");
