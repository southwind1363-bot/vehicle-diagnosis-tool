...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1183+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('function formatReadoutQualityReviewRequestSummary(summary, fallback = NO_DATA)') && appSource.includes('primaryReadoutQualityReviewRequest') && appSource.includes('read-only'), "OBD app should format readout quality review request summaries");
...
check(appSource.includes('const APP_VERSION = "2.457.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-13";'), "OBD app version should advance for readout quality review request display");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1183");
