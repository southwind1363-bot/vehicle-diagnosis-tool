...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(source.includes('requestPlanGateBlockedReasonCount: Number.isFinite(Number(readoutRequestPlanGateSummary.blockedReasonCount))'), "diagnostic flow summaries should expose request plan gate blocked reason count");
    check(source.includes('requestPlanGateActionRequired: readoutRequestPlanGateSummary.actionRequired === true,') && source.includes('requestPlanGateNextActionId: readoutRequestPlanGateSummary.nextActionId || null,'), "diagnostic flow summaries should expose direct request plan gate action fields");
    check(source.includes('requestPlanGateActionCount: Number.isFinite(Number(readoutRequestPlanGateSummary.actionCount))') && source.includes('requestPlanGateActionIds: Array.isArray(readoutRequestPlanGateSummary.actionIds)'), "diagnostic flow summaries should expose direct request plan gate action counts and ids");
    check(source.includes('requestPlanGateActionByReasonId: readoutRequestPlanGateSummary.actionQueueByReasonId') && source.includes('typeof readoutRequestPlanGateSummary.actionQueueByReasonId === "object"'), "diagnostic flow summaries should expose request plan gate actions by blocked reason");
    check(source.includes('requestPlanGateActionReadoutIds: Array.isArray(readoutRequestPlanGateSummary.actionReadoutIds)') && source.includes('requestPlanGateActionByReadoutId: readoutRequestPlanGateSummary.actionQueueByReadoutId'), "diagnostic flow summaries should expose request plan gate actions by blocked readout");
...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1426+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('const APP_VERSION = "2.505.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-14";'), "OBD app version should advance for queue cursor aliases");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1426");
