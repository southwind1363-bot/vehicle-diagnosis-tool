...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 1087+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
check(appSource.includes('function formatChangedIdReviewTargetLabel(reviewTarget = "")') && appSource.includes('request_plan_review: "review:request-plan"'), "OBD changed id display summaries should format review target labels");
...
check(appSource.includes('const APP_VERSION = "2.441.0";') && appSource.includes('const APP_LAST_UPDATED = "2026-07-13";'), "OBD app version should advance for labeled primary review target action ids");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 1087");
