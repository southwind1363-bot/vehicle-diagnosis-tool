...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(functionBody.includes('const pendingReadoutQueueSummary = {') && functionBody.includes('remainingAfterNextCount: Math.max(0, pendingReadoutQueue.length - 1)'), "buildCoreSessionStatus should expose pending readout queue summary");
    ...
    check(functionBody.includes('const capturedReadoutPercent = readoutStates.length') && functionBody.includes('capturedPercent: capturedReadoutPercent,'), "buildCoreSessionStatus should expose direct captured readout percent");
