...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(source.includes('item.comparison.actionRequiredChanged === true') && source.includes('item.comparison.actionIdsChanged === true') && source.includes('Number(item.comparison.actionQueueCountDelta || 0) !== 0'), "imported session section summaries should treat request plan action changes as gate changes");
    check(source.includes('item.actionRequiredChanged === true || item.nextActionChanged === true || item.actionIdsChanged === true') && source.includes('Number(item.actionQueueCountDelta || 0) !== 0'), "imported session top-level summaries should treat request plan action changes as gate changes");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 768");
