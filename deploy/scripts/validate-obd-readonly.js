...
const coreSessionStatusFunctionChecks = () => {
  ...
  if (coreSessionStatusFunctionSource) {
    ...
    check(source.includes('const changedIdDisplayRows = changedIdSummaries.map((item) => ({') && source.includes('rows: changedIdDisplayRows,'), "imported session comparison summaries should expose changed id display rows");
...
check(appSource.includes("const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze") && appSource.includes('validationCheckLabel: "OBD安全検証 920+件"'), "OBD progress overview should expose the diagnostic core validation snapshot");
...
if (failures.length) {
  ...
  console.log("OBD read-only safety checks: 920");
