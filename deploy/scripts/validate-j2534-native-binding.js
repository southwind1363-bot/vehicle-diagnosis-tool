...
async function main() {
  ...
  try {
    ...
    for (const platform of platforms) {
      ...
      for (const scenario of ["success", "open-failure", "overrun", "hang", "crash", "missing-open", "missing-read", "missing-close", "receive-success", "owned-receive", "request-abi"])
      {
      ...
      assert.ok(bindingResult.stdout.includes("DTC declared dependency loader gate: 24 rejected before OS loading"));
      total += 5;
  ...
  await import("./native/validate-mode01-pair.js");
