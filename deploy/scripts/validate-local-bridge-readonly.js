...
try {
  ...
  try {
    ...
    check(j2534Status.data.sample_mode === false && j2534Status.data.vci_detected_count === 2 && j2534Status.data.registration_status === "registered_driver_detected" && j2534Status.data.driver_readiness_status === "static_inspection_pending" && j2534Status.data.next_check === "verify_driver_library_path" && j2534Status.data.registry_roots_checked?.length === 2 && [32, 64].includes(j2534Status.data.bridge_runtime_bitness) && j2534Vci.data.driver_status === "j2534_registry_detected" && j2534Vci.data.driver_readiness_status === "static_inspection_pending" && j2534Vci.data.devices.length === 2 && j2534Vci.data.devices.every((item) => item.connected === false), "J2534 registry discovery did not expose detected drivers and static runtime context without opening a VCI");
