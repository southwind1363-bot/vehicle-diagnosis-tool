...
2026-06-12時点で、現行・履歴を合わせた車名候補341件、型式・エンジン型式まで持つ詳細候補313車種、年式範囲769件、DTC records 853件を登録しています。未登録の組み合わせは推測で補わず、手入力とメーカー資料確認へ戻します。
汎用DTCはP0839まで段階的に拡張し、基準電圧、警告表示、アクチュエータ電源、吸気マニホールド可変機構、グロープラグ、ECM電源リレー、冷却ファン、トランスミッション油圧・クラッチ相関・シフトレバーX/Y位置・手動シフトスイッチ・クラッチペダルスイッチ・四輪駆動切替スイッチの診断手順を収録しています。
