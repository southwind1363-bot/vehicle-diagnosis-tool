...
- JSONの保存形式・容量制限・処理中の保存制限は変更しません。取込時に記録した除外履歴の保存を確認していますが、詳細履歴が欠けて件数だけが残る不完全な入力を復元するものではありません。この場合、既存のJSON正規化でサマリーの無効件数などが残らないことがあり、往復保持は保証しません。正常値を推測で補ったり、履歴を別のDTC行へ割り当てたりしません。検証: `node scripts/validate-manufacturer-history-export.js`。
