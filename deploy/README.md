...
対象は共有PID正規化処理を通るブリッジ・JSON・CSV入力です。既に正規化済みの `monitorValues` を内部セッションAPIへ直接渡す経路の全面再検証や、取り込まれた過去の分析コメントの再判定ではありません。車種別正常範囲の追加、診断ロジックの簡略化、車両通信・消去・作動系の開放は行いません。
