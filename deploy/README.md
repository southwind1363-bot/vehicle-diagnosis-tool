...
3.13.333以降のPC配布版は `inspect-workstation-j2534.cmd` を開くと、登録ドライバー、DLLの32/64bit、静的検査結果と次の確認を日本語で表示します。VCI・車両の接続は不要です。配布版は内容検査に成功してから確認を開始します。DLL実行、車両通信、ファイル保存、外部送信はありません。静的検査済みでも実車適合・接続成功ではなく、実機通信機能の実装と試験は残っています。既存の `npm run inspect:j2534` のJSON出力は変更していません。

J2534の次工程として、接続・識別情報取得・終了の内部処理と模擬応答による異常系検証を追加しました。実DLLローダーや公開ブリッジには未接続で、実機通信の開放ではありません。検証は `npm run validate:j2534-lifecycle`、設計と残作業は [J2534内部接続処理](scripts/J2534-IDENTITY-LIFECYCLE.md) を参照してください。公開画面・配布版の版番号と進捗率は変更していません。

内部処理を固定の模擬ワーカーで実行する監視処理も追加しました。中止・タイムアウト・出力超過では終了要求後も子プロセスの終了確認まで二重起動を防ぎ、異常終了した出力を成功結果として採用しません。検証は `npm run validate:j2534-supervisor`。これは模擬環境の検証で、実DLLの読込・実機通信・ネイティブ終了処理の確認は未実装です。
