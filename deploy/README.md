...
- 起動時に表示されるペアリング値を、ブラウザ側の既存の詳細トークン設定と一致させます。値は外部共有しないでください。環境変数で指定しなければ起動ごとに生成され、ファイルには保存しません。
...
- 専用経路でも詳細トークンの照合と送信制限は同じです。既に詳細トークンを設定したブラウザでは、起動前に `LOCAL_BRIDGE_PAIRING_TOKEN` をその値に合わせてください。再起動ごとに生成される新しい値で既存の詳細トークンを解除・上書きすることはできません。
