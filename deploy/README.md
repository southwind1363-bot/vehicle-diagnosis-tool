...
3.13.348ではPC配布パッケージのコピー整合性をJ2534の一回限り操作へ統合しました。package-integrity.jsonとの一致を操作発行時とnative worker開始直前に再検証し、途中で不一致・欠落が発生した場合はworkerを起動せず拒否します。外部入力の真偽値では代用できず、ソースツリーには証拠がないため操作を発行しません。発行者真正性や実車適合の証明ではなく、ベンダーDLLのロード、J2534 API実行、車両通信はまだ行いません。進捗率は32%です。
