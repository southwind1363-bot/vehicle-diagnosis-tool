...
## 現在の完成版
3.13.411では、通常診断のECUスキャン結果で実応答と車両適用表の候補を分離しました。応答状態とDTC件数は正規化済みECU応答だけから表示し、適用表にある未観測ECUは故障や無応答と判定せず「適用候補・未観測」として表示します。診断判定、原因推定、セッション形式、JSON/CSV形式、読取コマンド、車両送信条件は変更していません。
