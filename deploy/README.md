...
OBD2車両読取タブは、将来のUSBシリアル接続によるDTC、フリーズフレーム、リアルタイムデータ表示専用です。現段階では安全検証中のため車両接続を無効にしています。外部診断機の読取結果を貼り付ける機能は診断補助へ統合し、現在は外部診断機の結果から診断を開始し、将来は車両読取結果を同じ診断補助へ直接引き継ぐ構成にしています。データモニター辞書は `data/obd-monitor-definitions.json` で管理し、エンジン、燃料、吸排気、排出ガス、過給、ディーゼル後処理、トルク、変速機、ハイブリッド、状態情報など134項目を登録しています。車両が対応して返した項目だけを表示し、未取得値は推測補完しません。実機接続を有効化する際も、DTC消去、アクティブテスト、ECU書換え、学習値初期化、セキュリティアクセスは許可しません。
