...
3.13.594: 開発専用J2534の保存結果は、入力源に「J2534開発検証データ（実車読取ではありません）」と表示します。保存形式や診断判定は変更せず、実機通信を有効にする更新ではありません。

3.13.593: Windowsの `verify-workstation.cmd` でも対象フォルダー等の追加引数を拒否します。引数なし、または待機を省く `--no-pause` のみ使用できます。確認したい配布フォルダー内のファイルを開いてください。

3.13.592: 配布物の検査スクリプトは追加引数を拒否します。別フォルダーを引数で指定しても検査対象にはなりません。確認したい配布フォルダー内の `verify-workstation.cmd` を開いてください。コピー整合性の検査であり、実VCI・実車の動作確認ではありません。

確認日: 2026-09-09。ローカル版: 3.13.566。最後の公開確認版は3.13.566です。最新の実装・確認状況は [開発計画](DEVELOPMENT-ROADMAP.md) を参照してください。下部の版別説明は過去の変更履歴です。

3.13.566では、相談文コピーが応答しない場合に約30秒で待機を終了し、操作ボタンを戻します。期限切れ後の代替コピー・GPTの自動表示は行いません。OS側の未完了コピー自体は取り消せないため、遅れてクリップボードが更新される可能性はあります。ブラウザー休止中は期限が遅れる場合があります。

3.13.565では、保存事例の読込エラーでCSV/JSON出力を止めたときや、CSV出力対象がないとき、保存通知を「開始しませんでした」に更新します。以前の保存開始表示を残しません。保存内容・出力形式・既存の警告や停止条件は変更していません。

3.13.564では、整備事例JSONのファイル読込が応答しない場合も30秒を目安に中止します。保存済み事例は保持し、選び直せるよう選択欄を解除します。自動再試行・ファイル変更・保存形式変更はありません。ブラウザー休止中は期限が遅れる場合があります。

3.13.563では、診断結果ファイルの読込が完了しない場合、30秒を目安に読込を中止します。元の読取結果を保持して保存操作へ戻し、再選択できるよう選択欄を解除します。ファイル自体は変更せず、自動で再読込しません。ブラウザー休止中はタイマーが遅れる場合があります。

3.13.562では、クリップボード読取が応答しない場合に30秒を目安にアプリの取込待機を終了し、手動貼付を案内します。現在の読取結果は保持し、遅れて届いた内容で置き換えません。OS側の読取要求や権限画面を取り消す機能ではなく、自動再試行・権限追加はしません。ブラウザー休止中はタイマーが遅れる場合があります。

3.13.561では、整備事例の未保存入力があるときに画面離脱の警告を要求します。入力を元に戻す・保存に成功する・確認してクリアする場合に解除します。下書きの自動保存ではなく、ブラウザーの警告制限や強制終了からの復元は保証しません。

3.13.560では、整備事例の保存前に電話番号・ナンバーらしい文字列を知らせる既存チェックの正規表現を修正しました。全個人情報の検出や匿名化を保証する機能ではなく、保存前の目視確認は引き続き必要です。保存可否・保存内容・形式は変更していません。

3.13.559では、最初の読取・ファイル取込中にも画面離脱の警告を要求します。結果のない処理を取り消した後は警告を解除し、取得済み結果がある場合の警告は維持します。警告表示はブラウザーの制限を受け、強制終了やOS停止からの復元を保証するものではありません。自動保存・通信・保存形式は変更していません。

3.13.558では、診断相談文コピーの旧式ブラウザー向け処理で、選択操作が失敗しても一時テキスト欄を残さないよう修正しました。相談文の内容・送信先・コピー権限・保存形式は変更していません。

3.13.557では、オフライン基盤がインストール済みで有効化前の場合に「切替待ち」と表示します。更新完了とは扱わず、読取中は画面を閉じず、終了後に結果を保存するよう案内します。更新停止そのものの修復、強制切替、自動再試行は追加していません。

3.13.556では、整備事例のJSON・CSV保存直前に端末内の保存内容と画面の読込時点を照合します。別タブなどで変更された場合は古い内容の保存を止め、「保存事例を再読込」を案内します。再読込後に内容を確認して保存してください。保存形式は変更せず、同時書込みを完全に防ぐロックではありません。

3.13.555では、実機サンプルTSVのダウンロード開始に失敗した場合、読取結果を保持して再操作を案内します。一時リンクとURLを後処理し、正常時も「保存を開始」と表示してディスク保存完了とは区別します。TSV内容・保存形式・車両通信は変更していません。

3.13.554では、診断結果ファイルの取込取消時に選択欄を空に戻し、同じファイルを選び直せるようにしました。同じ選択欄から別のファイルへ差し替える際は、新しい選択を保持します。取得済み結果・保存形式・車両通信は変更していません。

3.13.553では、診断結果ファイルの取込取消・差し替え時に、未完了のファイル読込処理を中止します。遅れて届く旧ファイルの結果は引き続き反映せず、取得済みの診断結果を保持します。取込形式・容量上限・車両通信は変更していません。

3.13.552では、保存事例の再読込失敗・別タブ変更を検知した際、以前の類似事例カードを残さず「未確認」表示へ切り替えます。正常に再読込できると最新の保存事例から再表示します。保存内容と類似度の計算方法は変更していません。

3.13.551では「事例入力をクリア」の前に確認を表示します。キャンセルすると入力は残り、実行すると入力だけが初期化されます。保存済み事例は削除しません。入力の自動保存やページ終了時の復元を追加したものではありません。

3.13.550では、整備事例の保存直前に別タブなどによる保存内容の変更を検知し、古い一覧での上書きを停止します。警告が出たら入力内容を控え、「保存事例を再読込」で最新の一覧を確認してください。入力フォームは保持され、自動再保存・自動マージはしません。完全な同時書込みロックではないため、事例の編集は1タブで行ってください。保存形式・車両通信は変更していません。

3.13.549では、整備データ読込失敗時の共通警告が読取結果の印刷から消える問題を修正しました。通常時は警告を表示せず、内蔵サンプルへ切り替わった場合のみ印刷にも警告を残します。実プリンター・OS印刷ダイアログ・別PC・実VCI/実車は未検証です。

開発時のブラウザー回帰検査は `npm run validate:scanner-browser` で実行します。診断結果のオフライン読込・再生・復旧・出力に続き、整備事例の1,000件取込、JSON復元、特殊ID表示、選択1件だけの削除、CSV失敗後の再試行を隔離プロファイルで検査します。Playwrightと対応ブラウザーの事前導入が必要です。`PLAYWRIGHT_MODULE` でモジュール、`PLAYWRIGHT_CHANNEL` でブラウザーを指定できます（既定は `chrome`、既存CIは `chromium`）。既存CIのスクリーンショット収集対象に事例検査も含め、バックアップJSON/CSVは収集しません。これは模擬データによる検査で、実車互換性の確認ではありません。

## まず保存済みの読取結果を確認する

1. [公開画面](https://tool.mukiguri.com/)を開くか、下記のPC配布版を起動します。
2. 利用前の注意を確認し、「OBD2車両読取」を開きます。パスワードが必要な場合は管理者から案内済みのものを使用します。不明な場合は管理者へ確認してください。
3. 「保存した読取結果を開く」から対応する読取結果JSONを選びます。結果画面では「読取結果ファイルを開く」も使用できます。取込結果と取得状態を確認し、基本結果から個別データへ進みます。
4. 保存可能な読取結果は「読取結果をJSON保存」で保存します。整備事例データは「データ管理」の「JSONバックアップ」で別途保存してください。
5. 紙などで確認する場合は「表示中の結果を印刷」を使用します。基本結果だけでなく、DTC・ライブ値・追加データ画面でも同じボタンを使用できます。対象はその時点の表示範囲だけで、絞り込みで非表示の項目や閉じた追加データは含みません。必要な詳細を表示してから印刷してください。再取込用のJSON保存とは別で、印刷・保存の完了はブラウザー側で確認します。PCのChrome/Edgeを対象にしています。

このファイル確認にはVCIや車両への接続は不要です。保存記録・模擬データの表示を、現在接続している実車からの取得や実車適合の証明として扱わないでください。取得失敗・未取得・正常なコード0件は別の状態です。

整備事例のJSONバックアップは、データ管理画面に保存開始または開始失敗を表示します。「開始」はファイル保存の完了を意味しません。ブラウザーのダウンロード一覧で完了を確認し、失敗時は端末内データを削除せず保存設定・空き容量を確認してください。

## PC配布版の起動・更新

Node.js 22以降とnpmを事前に用意します。配布ZIPを全展開し、同梱のREADME.txtを読んで `verify-workstation.cmd`、`start-workstation.cmd` の順に開きます。画面が自動で開かなければ、起動画面のURLを開いてください。終了は起動画面で `q` とEnterです。Node本体・VCIドライバー・個人の保存データは同梱していません。

更新前は整備事例と読取結果を別々にJSON保存し、旧版を残して新版を別フォルダーへ全展開してください。同じブラウザープロファイルと同じURL（ホスト名・ポート番号を含む）を使います。URLを変えるとブラウザー保存領域も別になります。失敗時にサイトデータ・キャッシュ・元のログを削除しないでください。詳しい復帰方法は配布README.txtに記載しています。

配布ZIPは別途受け取る成果物です。このリポジトリのソースZIPを、そのまま依存ライブラリ同梱の配布物として扱わないでください。開発者が配布物を生成する場合は、依存ライブラリ導入済みのdeployで `npm run package:workstation` を使用します。既存出力があれば上書きせず停止します。

通常の接続画面でも「通信設定（ELM327 / Web Serial）」から、VCIの説明書に記載された通信速度を選べます。設定は詳細画面と共通です。接続開始後は変更できず、切断完了後に再設定できます。速度の自動探索や自動再接続は行いません。

## 実機で使う前の確認

新規購入するVCIの開発基準と資料上の候補は [VCIの選定・受入条件](VCI-REQUIREMENTS.md) に整理しています。購入前の条件と、実機確認済みの事実を分けて扱います。

- PCのELM327読取処理は実装されていますが、手持ちVCIのシリアル経路・型番と対象車両での確認は別途必要です。すべてのBluetooth機器を接続可能とはしていません。
- ローカルのJ2534ブリッジは静的確認用です。起動成功やドライバー検査合格だけでは、実車読取が利用可能になったことを意味しません。
- iPhoneは署名済みアプリの導入と実機BLE確認、THINKCARは直接接続の実装・互換確認が残っています。
- DTC消去・サービス実行は利用可能な機能として案内しません。自動再試行や車両通信の権限を、この起動手順で拡張しないでください。

別PC、実VCI、対象車両の適合は未確認です。また、自動操作環境ではブラウザー再起動後の再ダウンロード時にプロセスが終了する問題をアプリなしの最小画面でも確認しています。手動操作で同じ問題が出るかは未確認であり、解決済みとはしていません。

## 過去の変更履歴

3.13.436では、次の読取候補の安全サマリーで未取得・空欄・真偽値・負数・小数の件数を0件として扱わないようにしました。総数、safe/unsafe、送信候補、実行候補の全件数が非負整数として取得済みで、内訳と上限が整合する場合だけ安全状態を表示します。不完全・矛盾データは「登録データなし」に退避します。読取計画、診断判定、保存形式、許可コマンド、車両送信条件は変更していません。

3.13.435では、ECU適合の内訳表示で外部セッションの未取得・空欄・真偽値・小数・一覧件数を超える観測件数を実測値として採用しないようにしました。0から表示対象ECU数までの整数または10進整数文字列で、各ECUの観測状態から集計した件数と一致する場合だけ明示件数を使い、それ以外は個別観測から再集計します。ECU適合判定、診断分岐、読取解析、保存形式、車両送信条件は変更していません。

3.13.434では、Web Serial接続の初期化結果で未取得・空欄・真偽値・不正形式の通信速度を「0 bps」と誤表示しないようにしました。初期化サマリーと同じ1200～1000000 bpsの整数または10進整数文字列として確認できた通信速度だけを表示します。初期化手順、許可コマンド、診断判定、読取解析、保存形式、車両送信条件は変更していません。

3.13.433では、DTC一覧の関連フリーズフレームでも未記録番号を「#0」と誤表示しないようにしました。既知番号と未記録が混在する場合も両方を表示します。UDS拡張データの件数表示で空欄・型不正・範囲外のレコード番号を取得済みとして数えないよう、起点DTCと共通の表示用番号チェックを使います。DTC照合、診断分岐、読取解析、保存形式、車両送信条件は変更していません。

3.13.432では、フリーズフレームの起点DTCで番号未記録を「#0」と誤表示する問題を修正しました。明示された0番を含む0～255の整数と10進整数文字列だけを番号として表示し、未記録・空欄・型不正・範囲外は「FF番号未記録」と表示します。起点DTC、状態、ECU情報は保持し、表示用整形で元データは変更しません。診断判定、読取解析、保存形式、車両送信条件は変更していません。

3.13.431では、診断画面の段階選択と車両設定から接続画面への移動時に、表示先の見出しへキーボードフォーカスを移します。非表示になった元のボタンに操作位置を残さず、ロック中・非表示画面へのフォーカス移動は行いません。読取中の自動再描画では操作位置を奪いません。診断判定、通信処理、保存形式、車両送信条件は変更していません。

3.13.430では、通常診断画面の接続表示を、機器選択・接続・初期化・読取・終了処理・終了未確認で区別しました。初期化中を接続済み、終了処理中を未接続と表示しません。THINKCARのiPhone接続準備にELM327用の検証状況が表示される問題を修正し、接続前の案内もWeb Serial・ブリッジ準備・iPhone未実装の経路別に表示します。表示だけの変更で、通信処理、診断判定、保存形式、車両送信条件は変更していません。

3.13.429では、表示中の画面より新しい有効なオフライン基盤を検出した際、共通ヘッダーに両方の版と再読み込み前の接続終了・保存の注意を表示します。同版・旧版・不正な版・キャッシュ名不一致は更新案内に使いません。画面の自動再読み込み、通信、データ保存、キャッシュ削除は追加しません。診断判定、保存形式、車両送信条件は変更していません。

3.13.428では、Windows配布版の起動時に車両診断画面を直接開くようにしました。手動で開くURLも同じ画面を指します。画面ロックと利用前の注意は維持し、自動接続・読取・ロック解除は行いません。通常のトップURLは従来どおり診断補助を表示します。診断判定、保存形式、車両送信条件は変更していません。

3.13.427では、VCIの終了未確認状態でも、全終了処理の試行が終わった後は取得済み結果をJSON保存できるようにしました。読取・書込み・接続・取込・終了処理が未決着の場合は引き続き保存を禁止します。保存通知にも終了未確認と再接続禁止を明示し、保存で隔離を解除しません。模擬VCIで接続、DTC読取、終了失敗、実際のJSON生成、再読込まで検証し、取得済みDTCと元の結果の保持、追加送信なしを確認します。保存形式、車両送信条件は変更していません。実機・実車試験は未実施です。

途中結果の検証で、複数ECUの保存・保留・永久DTC応答を一括decodeすると別行の長さ・サービスバイトがDTCとして混入し、ECU帰属も失われる問題を修正しました。応答を既存classifierで分離し、ECUごとにdecodeしてから統合します。診断の推定規則は変更せず、読取DTCとECU帰属の誤りを正します。NO DATA・未取得・陰性応答の区別を維持し、不完全フレームは未解析として扱います。旧版で保存済みの誤ったDTCは自動削除しません。同じコードが実際に存在する可能性があるため、元の通信記録か再読取で確認する必要があります。

3.13.426では、Web Serialの通常切断でreaderのcancel・ロック解除、writerのロック解除、portのcloseが失敗した場合も「終了未確認」として扱うよう修正しました。従来は書込み待機中だけが対象でした。取得済み結果を維持し、終了未確認中は再接続とローカルブリッジへの切替を禁止します。ロック解除や切断の再操作でこの隔離を解除しません。接続取消後に遅れてopenが完了したポートのclose失敗も隔離し、初期化へ進みません。cancelのみが失敗し後続closeが成功した場合も、保守的に終了未確認を維持します。模擬VCIの実接続・読取経路で、未解放ロックとclose失敗、結果保持、新たな機器選択・接続・送信の阻止を検証します。診断判定、保存形式、送信許可リスト、消去・制御の無効状態は変更していません。実機互換性と実車安全性は未確認です。

3.13.425では、Web SerialのCRのみの応答で行区切りが消える問題を修正しました。CR/CRLF/LFを同じ行構造へ統一し、エコーとエラー、複数ECU、正常応答と否定応答を別行のまま既存パーサーへ渡します。診断アルゴリズムや保存形式は変更しませんが、これまで連結されていた応答の分類・読取結果は正されます。PC用ELM327の実装済み読取と、消去・作動系の実行通信未実装を表示で区別しました。送信許可リスト、認証、消去・制御の無効状態は変更していません。

実車なしの結合試験は `npm run validate:serial-integration` で実行できます。模擬Web Serialポートのバイト列を、実際の接続・初期化・識別・基本読取・解析・セッション保持・切断処理へ渡します。通常操作者のロック解除状態で、詳細トークンを使わずに試験します。CR/CRLF/LFと分割応答、複数ECUのDTC帰属、対応PIDだけの要求と1726rpmの復号、空FF・readiness・ECU情報・Mode06、PID無応答、一部取得後の切断、初期化失敗、消去拒否を確認します。模擬試験の成功は実VCI互換性や実車安全性の確認ではなく、画面の実機未確認表示は維持します。既存の `validate:serial` と `validate:obd` からもこの結合試験を実行します。

3.13.424では、追加データ内でフリーズフレーム・レディネス・ECU情報・対応PID・Mode06・ECU応答を選択表示できるようにしました。対象の詳細がない場合も項目名付きの空表示を維持し、全読取情報へ戻せます。読取更新後も選択項目を保持します。ECU一覧からの詳細表示がDTC画面に隠れる問題を修正し、ロック中の移動・検索変更も防ぎます。診断判定・保存形式・車両送信条件は変更していません。

3.13.423では、通常の個別データ画面をDTC・ライブ値・追加データの選択表示に変更しました。選択していない区分を同時に並べず、空結果でも該当区分の取得状態を表示します。表示中の区分を強調し、詳細モードと診断補助側の一覧は従来どおり保持します。診断判定・保存形式・車両送信条件は変更していません。

3.13.422では、対応PIDの在庫キー、報告済み/未解決ECU範囲、前回セッション比較をnetwork bus・channel・gateway route別に分離しました。同じECUアドレスでも別経路の対応PIDを混同せず、旧2項目キーと新しい経路付き5項目キー、混在・不正キーは比較停止します。partial読取は双方で報告済みの同一ECU経路だけを比較し、29bit ECU ID、JSON往復、車両送信無効を維持します。
...
3.13.415では、多ECU車の車種別適合一覧を切り捨てず扱えるよう、supported_ecus の上限をruntime・データ検証・診断コアの3層で20件から64件へ拡張しました。64件は期待ECUと保存JSONへ全件保持し、65件以上は切捨て状態として期待ECUへの昇格を拒否します。4MBの保存上限を維持するため、新規exportではコアから再構築できる診断フロー・解析準備の重複要約を省きます。未観測ECUを無応答や故障へ変換せず、保存JSON再取込時の証跡信頼降格と車両送信無効を維持します。

3.13.414では、車種別ECU適合一覧を年式候補データから分離した固定同梱カタログとして追加しました。メーカー・車種・型式・年式・エンジン・市場が一意に完全一致し、検証済みHTTPS出典、証跡ID、資料位置、完全一覧、安全フラグが揃う場合だけ期待ECUへ供給します。現在のカタログは根拠待ちのため0件で、汎用OBD読取の動作は変えません。不正、複数一致、部分一覧、21 ECU以上は全件不採用とし、車両送信は引き続き無効です。

3.13.413では、車両適用情報とECU一覧専用証跡を分離し、メーカー・車種・型式・年式・エンジン・市場の完全一致、検証済み出典、証跡ID、資料位置、一覧範囲が揃う場合だけ supported_ecus を期待ECUへ昇格するコアゲートを追加しました。保存JSONの安全なECU記述と証跡は保持しますが、自己申告のexport形式では信頼を引き継がず再検証必須へ降格します。未検証・競合・不完全・別仕様・切捨て一覧は期待ECUにせず、未観測から無応答や故障を推定しません。車両送信は引き続き無効です。

# 自動車整備 診断補助ツール

開発の現在位置・段階公開・完了条件は [診断機完成までの開発計画](DEVELOPMENT-ROADMAP.md) を参照してください。自動検証件数と実車対応実績は分けて扱います。

登録済みJSONデータ、問診、整備事例を使って、確認順序を整理する診断補助ツールです。

## 現在の完成版
3.13.412では、通常診断のECUスキャン結果へ「すべて・応答/観測・明示的無応答・未観測候補・適用未確認」の状態別絞り込みを追加しました。実応答と適用確認は重複する事実として保持し、未観測から無応答や故障を推定しません。診断判定、原因推定、セッション形式、JSON/CSV形式、読取コマンド、車両送信条件は変更していません。
3.13.409では、通常診断画面を「車両選択・接続・全体結果・個別データ」の1画面1操作フローへ分離しました。車両、VCI、接続状態を常時表示し、接続操作は専用画面、主要結果は要約画面、DTC・ライブデータ・フリーズフレーム等は個別データ画面で選択します。開発設定、通信契約、ロードマップは詳細モードへ保持しています。既存の接続・基本読取・切断処理を再利用し、診断判定、原因推定、セッション形式、JSON/CSV形式、読取コマンド、車両送信条件は変更していません。

3.13.408では、かんたん画面のレディネス結果へ完了・未完了・非対応・状態不明・未取得の内訳を追加しました。既存レディネス要約を再利用し、複数ECU応答は集約判定せずECU別系統数を表示します。検査合否を新たに判定せず、診断判定、セッション形式、読取コマンド、車両送信条件は変更していません。

3.13.407では、かんたん画面のフリーズフレーム結果へ起点DTCを最大3件表示しました。正規化済み起点DTC一覧と単一起点の両方を受け取り、重複を除外し、P0000は起点なしとして表示対象に含めません。ECU・フレーム番号等の詳細は従来の詳細画面に保持し、診断判定、原因推定、セッション形式、読取コマンド、車両送信条件は変更していません。

3.13.406では、かんたん画面のDTC結果へ保存・保留・永久の読取範囲を追加しました。状態別件数に加えて取得済み、件数応答、空応答、未読取を既存DTC状態契約から表示し、0件と未読取の混同を防ぎます。診断判定、原因推定、セッション形式、読取コマンド、車両送信条件は変更していません。

3.13.405では、かんたん画面のDTC結果へ保存・保留・永久・不明の状態別件数を追加しました。既存の正規化済みDTC状態だけを集計し、未分類コードは保存扱いにせず「不明」と表示します。診断判定、原因推定、セッション形式、読取コマンド、車両送信条件は変更していません。

3.13.404では、かんたん画面の基本読取結果へ対応PIDとMode06を追加しました。既存readout coverage契約を状態の正本とし、正規化済み件数または配列件数を表示して各詳細へ直接移動できます。空応答と未取得は区別し、診断判定、セッション形式、読取コマンド、車両送信条件は変更していません。

3.13.403では、読取結果内の固定ナビゲーションへ「↑ 結果」を追加しました。DTC、ライブデータ、フリーズフレーム、レディネス、ECU情報の確認中も、基本読取結果へ1操作で戻れます。診断判定、セッション形式、読取コマンド、車両送信条件は変更していません。

3.13.402では、かんたん画面の主要読取6項目を押しやすい結果ボタンへ変更しました。DTCとライブデータは各一覧へ、フリーズフレーム、レディネス、ECU情報は該当する詳細カードへ直接移動し、詳細が空の場合は取得状態へフォールバックします。診断判定、セッション形式、読取コマンド、車両送信条件は変更していません。

3.13.401では、かんたん画面のDTC、フリーズフレーム、ライブデータ、レディネス、ECU情報へ、取得済み、応答0件、未取得、読取中の状態を追加しました。状態は既存readout coverage契約を正本とし、基本読取中の項目だけ一時状態で示します。一時IDは保存せず接続・切断時に消去し、診断判定、セッション形式、読取コマンド、車両送信条件は変更していません。

3.13.400では、かんたん画面の診断結果から基本読取を再実行し、接続を安全に切断できる操作を追加しました。結果表示へ自動移動した後も接続・初期化・読取・停止理由を常時表示し、通信中は再読取と切断を既存busy判定で無効化します。既存のread-only主操作と切断処理を再利用し、診断ロジック、保存形式、読取コマンド、車両送信条件は変更していません。

3.13.399では、かんたん画面の診断結果上部に、DTC、フリーズフレーム、ライブデータ、レディネス、ECU情報、主要読取率の6項目を表示しました。未取得と応答0件を分け、DTC保存案内と未完了項目数を簡潔に示します。大量の診断セッション詳細は詳細モードへ残し、診断ロジック、保存形式、読取コマンド、車両送信条件は変更していません。

3.13.398では、かんたん画面に接続・初期化・基本読取・切断の現在状態を常時表示し、失敗時は既存のエラー表示も同期するようにしました。詳細画面へ移らなくても読取の進行と停止理由を確認できます。診断ロジック、読取コマンド、保存形式、車両送信条件は変更していません。

3.13.397では、かんたん画面の主ボタンを、車両選択後のロック解除案内、PC版ELM327接続、基本一括読取へ状態連動させました。基本一括読取は保存・保留・永久DTC、フリーズフレーム、レディネス、ECU情報、Mode06、ライブデータの既存read-only許可リストだけを使います。簡易切断を追加し、詳細モードへ切り替えた場合は簡易接続を終了します。個別コマンド、ブリッジ操作、書込み系は詳細ロックまたは送信無効のままです。

3.13.396では、診断画面の初期表示を「かんたん」に整理し、車両・接続と診断結果の2段階だけで進められるようにしました。進捗、読取プレビュー、個別読取、通信設定、開発ロードマップは「詳細」へまとめ、切替後は従来機能をすべて利用できます。読取専用の安全境界、診断データ、保存形式、車両送信無効は変更していません。

3.13.395では、ECU観測差分から再確認する通信経路候補ごとに、端末選択、VCI識別、実機適合、車両・ECU適合、対象ECU、通信方式、診断アドレス、read-only境界の確認項目を追加しました。ELM327はiPhoneネイティブホストとPC Web Serialの条件を分離し、J2534はDLL識別と32/64bit適合を明示します。全項目は未確認から開始し、経路確認0件として保存・表示するだけで、自動選択、実行、車両送信は無効のままです。

3.13.394では、ECU観測差分から再確認する際の通信経路候補を安全に分離しました。標準OBDのエンジン系ECUだけにELM327候補を示し、ABSなど多ECUとUDS/29bitアドレスはJ2534対象ECU候補として扱います。候補は実機・車種・ECU適合未確認、自動選択なし、read-only、実行無効としてJSON再取込後も保持し、車両送信は有効化しません。

3.13.393では、前回セッションから未観測になったECUと応答アドレスが変化したECUを、対象ECU情報の再確認計画へ結び付けました。対応ECU一覧自体の追加・削除は適合資料の確認へ分離し、ECU情報読取候補は対象選択待ち、read-only、実行無効、自動対象選択無効として保存します。ELM327のMode 09とJ2534/UDSの対象ECU読取を同一扱いせず、車両送信は無効のままです。

3.13.392では、保存済みセッションと現在セッションの対応ECU観測を安定識別キーで比較し、ECU候補の追加・消失、観測有無、応答アドレスの変化を診断セッションへ保持するようにしました。旧JSONに識別キーがない場合も既存規則で再構築し、配列順の違いを差分にしません。詳細画面と診断フローへ事実差分を表示し、適合判定、原因推定、車両通信は変更せず送信無効を維持します。

3.13.391では対応ECUごとの観測結果へ、システム名、ECU名、診断アドレス、通信方式から生成する安定識別キーを追加しました。完全に同じECU行が重複した場合も連番で一意に区別し、camelCase/snake_caseの両形式とJSON再取込で維持します。将来の前回セッション比較の土台となる加算フィールドで、適合判定、DTC/PID解析、既存JSON読取、車両通信は変更せず送信無効を維持します。

3.13.390では対応ECU一覧の観測結果を、詳細セッションと診断フローパネルの両方へECU名・診断アドレス・観測アドレス付きで表示するようにしました。観測件数を明示し、画面上は最大3件に制限して残件数を表示します。未観測は故障と断定せず事実表示に留め、適合判定、保存形式、DTC/PID解析、車両通信は変更せず送信無効を維持します。

3.13.389では対応ECU一覧の各行について、システム名、ECU名、通信方式、診断アドレスを実際に観測したECUアドレスへ結び付けて診断セッションへ保存するようにしました。ECUごとの観測有無と集計件数を保持し、範囲指定、部分観測、JSON再取込にも対応します。単一ECU指定が優先される既存動作、適合判定、DTC/PID解析、車両通信は変更せず送信無効のままです。

3.13.388では複数ECUの適合照合証拠を、詳細セッションと診断フローパネルの両方へ表示しました。一致、一部観測、要確認、応答未観測、比較不可、未設定を区別し、対応ECU一覧または単一ECUの根拠、一致・未観測・適合外応答を最大4件まで表示します。表示だけの変更で、適合判定、診断結果、保存形式、車両通信は変更せず送信無効を維持します。

3.13.387では複数ECUの適合照合について、期待アドレスごとの一致・未観測、照合可能な観測アドレスの一致・不一致、件数、部分観測状態を診断セッションへ保持するようにしました。範囲指定は範囲単位で集計し、1件でも一致した従来のmatched判定や再読取優先度は変更しません。JSON再取込後も証拠を維持し、DTC/PID解析と車両通信は変更せず送信無効のままです。

3.13.386では車両適合に単一ECUアドレスが未設定の場合、対応ECU一覧の診断アドレスを実読取ECUとの照合根拠に使えるようにしました。単値と同じ長さの16進範囲を扱い、照合根拠と期待アドレス一覧を要約へ保持します。単一ECUアドレスがある既存セッションは従来どおり最優先し、適合不一致は確認候補のまま、DTC/PID解析と車両通信は変更せず送信無効を維持します。

3.13.385では車両適合の対応ECU一覧について、システム名・ECU名・通信方式を長さ制限付き文字列に限定し、不正な先行別名を飛ばして正常な別名を採用するようにしました。診断アドレスは0x付き表記を含む16進単値または範囲だけを保持し、構造化値、曖昧な数値、無効記号を除外します。元件数と保持件数、切捨て状態はJSON再取込後も維持し、既存のECU照合、DTC/PID解析、車両通信は変更せず、送信無効のままです。

3.13.384では車両適合の出典名・URL・日付・証拠IDについて、先行する不正値を飛ばして最初の安全な別名を採用するようにしました。候補範囲・適用範囲にも同じ規則を適用し、認証情報、クエリ、フラグメントは保持しません。confidenceは文字列数値を含む0〜1の有限値だけを保存し、範囲外や構造化値は未登録にします。正常な適合根拠はJSON再取込後も保持し、DTC/PID解析と車両通信は変更せず、送信無効を維持します。

3.13.383では車両適合のメーカー・車名・型式・年式・エンジン・駆動方式・対象ECUなどの識別フィールドを型確認と長さ上限付きで正規化しました。不正なオブジェクトや配列は保存せず、先頭の不正値が後続の正常な別名を遮らないようにしています。不正値だけでは車両情報・適合根拠ありと判定せず、正常な文字列と数値年式、DTC/PID解析、車両通信は維持し、送信無効のままです。

3.13.382では車両適合の候補範囲・適用範囲に含まれる出典情報を安全化しました。出典名・日付・証拠IDを型確認と長さ上限付きで正規化し、認証情報付きURL、クエリ、フラグメント、オブジェクト値を保持しません。verified系フラグが矛盾する場合は検証済み扱いを解除し、競合状態をJSON再取込後も保持します。候補範囲自体、適合status、DTC/PID解析、車両通信は変更せず、送信無効を維持します。

3.13.381では車両適合資料のトップレベル出典フィールドを安全化しました。出典名・日付・証拠IDを型確認と長さ上限付きで正規化し、HTTP(S)以外、認証情報付きURL、クエリ、フラグメント、オブジェクト値を保持しません。不正な出典だけの入力は根拠なしの非ブロッキングreviewへ移し、JSON再取込後も送信無効を維持します。正常な資料、適合status、DTC/PID解析、車両通信は変更していません。
...
3.13.362ではローカルbridgeのECU別UDS負応答・pendingから、target ECUとsource ECUが11bitまたは29bitの正しい要求/応答ペアで一致し、応答回数と待機時間が明示された単一証跡だけをtransport lifecycleへ接続しました。不一致ECUと複数ECUの曖昧な証跡は採用しません。raw応答非保持、adapter未実装、dispatch無効、車両コマンド無効を維持し、UDS/CAN FDソフト進捗を58%へ更新しました。
...
公開前の一括検証は `npm.cmd run validate:release` です。OBD（シリアルの寿命管理、セッション出力、整備事例の保存・CSV/JSON入出力、接続案内と状態表示を含む）、ブリッジ、J2534の寿命管理と監督処理、ローカル起動と画面操作、配布物、オフライン資材、診断辞書の順に実行し、失敗した段階で停止します。整備事例の保存と接続案内のテストはOBD総合検証から呼ぶため、既存のGitHub検証でも実行されます。模擬ストレージを使用し、利用者の保存データや車両にはアクセスしません。
