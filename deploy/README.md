...
移行先にNode.jsとnpmを事前導入し、作成されたフォルダー全体をUSB等で移してください。`start-workstation.cmd` を開けば、追加の `npm install` やインターネット接続なしでPC版を起動できます。移行先のNodeの版・32/64ビットとVCI適合は別途確認が必要です。実車対応や車両送信の権限は変更しません。
