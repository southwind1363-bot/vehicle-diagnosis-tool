...
3.13.335以降のPC配布版は `inspect-workstation-j2534.cmd` を開くと、登録ドライバー、DLLの32/64bit、静的検査結果と次の確認を日本語で表示します。VCI・車両の接続は不要です。配布版は内容検査に成功してから確認を開始します。DLL実行、車両通信、ファイル保存、外部送信はありません。静的検査済みでも実車適合・接続成功ではなく、実機通信機能の実装と試験は残っています。既存の `npm run inspect:j2534` のJSON出力は変更していません。
