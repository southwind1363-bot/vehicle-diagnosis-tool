...
## 現在の完成版
3.13.410では、通常診断の全体結果へ応答ECU一覧を追加しました。正規化済みECU応答要約を正本に、ECU名・アドレス・応答状態・報告DTC件数・取得サービスを表示し、ECU選択時は既存のECU応答詳細を同じ識別子で絞り込みます。DTC件数が未記録の場合は0件へ補完せず、ECU別応答未取得も正常とは判定しません。診断判定、原因推定、セッション形式、JSON/CSV形式、読取コマンド、車両送信条件は変更していません。
