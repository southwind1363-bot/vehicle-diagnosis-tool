...
確認日: 2026-09-08。Webアプリ版: 3.13.538。最新の実装・確認状況は [開発計画](DEVELOPMENT-ROADMAP.md) を参照してください。下部の版別説明は過去の変更履歴です。
...
5. 紙などで確認する場合は「表示中の結果を印刷」を使用します。対象はその時点の表示範囲だけで、絞り込みで非表示の項目や閉じた追加データは含みません。必要な詳細を表示してから印刷してください。再取込用のJSON保存とは別で、印刷・保存の完了はブラウザー側で確認します。PCのChrome/Edgeを対象にしています。
