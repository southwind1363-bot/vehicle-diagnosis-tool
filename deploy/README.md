...
起動時のウィンドウには今回のペアリング値を表示するため、その内容を外部共有しないでください。起動ウィンドウは利用中そのまま残し、終了は `Ctrl+C` です。コマンドから使う場合は次の方法も引き続き利用できます。
...
- 終了は `Ctrl+C` です。起動途中でポート競合が起きた場合、今回起動したサーバーだけを終了します。他のプログラムは停止しません。
