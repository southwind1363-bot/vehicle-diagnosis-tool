...
不存在はconflict/record_not_found、不一致はconflict/record_mismatchで削除しない。入力不正はrejected、期限超過・削除失敗・事後確認不能等はindeterminateとする。5秒の期限と後続処理の停止を維持し、DB作成・移行・一括削除・期限による自動削除・再試行は行わない。保存schemaは変更せず、saveの追加専用動作も維持する。

画面は検証して読み込んだ選択記録だけを、IDと取り消せない旨を明示して利用者が確認した場合に削除する。比較候補は明示解除前に削除しない。確認前後の記録参照・選択世代・ID・JSON・保存時刻・画面権限・通信状態を照合する。削除中のbusyは閉じ直しやロックでも保持し、保存・一覧・読込・照合・再削除を開始しない。成功時は該当選択と直前保存IDを失効し、不明/拒否/競合時は再読込を要求する。診断結果を変更せず、車両通信も行わない。

候補保護は現在のタブ内に限る。別タブの候補や将来の実行が必要とする記録を保護する予約・排他制御ではない。将来の車両実行には独立した予約と所有権確認が必要であり、保存・削除の確認を実行許可に転用しない。この機能で利用者の実データを開発作業中に削除することはない。
