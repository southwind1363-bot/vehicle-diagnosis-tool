...
const OBD_INTERFACE_PROGRESS = Object.freeze({
  ...
  uds_canfd: Object.freeze({
    progressPercent: 77,
    currentBasis: "read-only DID / ECU情報、拡張DTC、NRC、ISO-TPログ取込、DID要求マニフェスト、bridge ECU範囲一致、transport resultからpositive / negative / pending / terminal completion manifestへのfail-closed変換、attempt ID付き非実行ライフサイクルまで実装済み。実transportアダプターは未実装。",
    nextBuild: "J2534/CAN transport workerのsanitized受信結果をuds_read_transport_result_v1として出力し、この変換境界へ接続する。",
...
const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze({
  ...
  bridgeValidationCheckLabel: "bridge検証 380件",
  recentMilestone: "UDS transport結果を検証済みcompletion manifestへ変換",
  scopeNote: "自動検証件数は実車確認済み車種数や完成率ではありません"
});
const APP_VERSION = "3.13.367";
