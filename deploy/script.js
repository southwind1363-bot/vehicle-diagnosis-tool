...
const cancelCaseImportButton = document.querySelector("#cancelCaseImportButton");
...
const obdOperationJournalRemoveStatus = document.querySelector("#obdOperationJournalRemoveStatus");
...
function initializeObdReadoutFilter(grid, prefix, searchKey, unit, options = {}) {
  ...
  if (selection) {
    grid.addEventListener("change", (event) => {
      ...
      if (target?.dataset?.monitorSelect === undefined) return;
      const focused = document.activeElement === target;
      const cards = Array.from(grid.children);
      const index = cards.findIndex(card => card.querySelector("[data-monitor-select]") === target);
      refresh();
      if (focused && index >= 0 && cards[index].hidden) {
        const remaining = [...cards.slice(index + 1), ...cards.slice(0, index).reverse()];
        const next = remaining.find(card => !card.hidden)?.querySelector("[data-monitor-select]");
        (next || input).focus();
      }
    });
...
function initializeObdStatusDisclosures() {
  document.querySelectorAll(".obd-status-details").forEach((details) => {
    ...
    window.addEventListener("beforeprint", () => {
      ...
      printState = { open: details.open, scrollTop: body?.scrollTop || 0, status: statusSnapshot() };
      details.open = true;
    });
    window.addEventListener("afterprint", () => {
      if (!printState) return;
      // Do not hide a warning that arrived while print preview was open.
      if (printState.status === statusSnapshot()) {
        details.open = printState.open;
        const body = details.querySelector(".obd-status-body");
        if (body) body.scrollTop = printState.scrollTop;
      }
      printState = null;
    });
...
const obdOperationJournalState = {
  ...
  recordIds: [],
  selectedRecord: null,
  selectedRecordOrigin: null,
  status: "一覧未取得",
  error: false,
  listing: false,
  loading: false
};
const obdOperationJournalSaveState = {
  promise: null,
  phase: "idle",
  id: null,
  status: "",
...
const obdOperationJournalRemovalState = {
  promise: null,
  phase: "idle",
  id: null,
  status: "",
...
caseResetButton.addEventListener("click", () => {
  if (!confirm("入力中の事例をクリアしますか？未保存の入力は元に戻せません。保存済みの事例は削除しません。")) {
    caseStatus.textContent = "入力のクリアをキャンセルしました。入力内容は保持しています。";
    return;
  }
  caseForm.reset();
  setDefaultCaseDate();
  updateCaseQualityPreview();
  caseStatus.textContent = "事例入力をクリアしました。保存済みの事例は変更していません。";
});
...
document.querySelector("#obdSessionOpenButton")?.addEventListener("click", openObdSavedReadout);
document.querySelector("#obdHomeOpenSessionButton")?.addEventListener("click", openObdSavedReadout);
...
obdOperationJournalCompareCurrent?.addEventListener("click", () => { void compareCurrentObdReadoutToOperationJournalRecord(); });
...
async function fetchJson(path) {
  ...
  try {
    ...
    // Keep the deadline active while receiving and parsing the response body.
...
function initializeLaunchView() {
  if (window.location.hash === "#obd-panel") activateTab("obd-panel");
}
...
function setObdUiMode(mode = "simple") {
  ...
  renderObdDeveloperGate();
  activeObdStage = obdUiMode === "simple" ? "home" : "details";
  if (obdAccessUnlocked) setObdStage(activeObdStage);
  else renderObdStageView(activeObdStage);
}
...
function openObdSavedReadout() {
  if (!obdAccessUnlocked || !obdImportFileInput) return;
  setObdStage("results");
  scrollToObdSection("obdImportStatus");
  obdImportFileInput.click();
}
...
function getObdEntryStage(autoStage, mode) {
  return mode === "simple" && autoStage === "setup" ? "home" : autoStage;
}
...
function clearObdOperationJournalViewer() {
  ...
  setObdOperationJournalSelectedRecord(null, null);
...
function setObdOperationJournalSelectedRecord(record, origin) {
  if (obdOperationJournalState.selectedRecord !== record || obdOperationJournalState.selectedRecordOrigin !== origin) {
    clearObdOperationJournalComparison();
  }
  obdOperationJournalState.selectionGeneration += 1;
  obdOperationJournalState.selectedRecord = record;
  obdOperationJournalState.selectedRecordOrigin = origin;
}

function resetObdOperationJournalRemovalOutcome() {
  if (obdOperationJournalRemovalState.promise) return;
  obdOperationJournalRemovalState.phase = "idle";
  obdOperationJournalRemovalState.id = null;
  obdOperationJournalRemovalState.status = "";
  obdOperationJournalRemovalState.outcome = null;
}

function rememberObdOperationJournalRemovalOutcome(result, expected) {
  const status = typeof result?.status === "string" ? result.status : "indeterminate";
  const reason = typeof result?.reason === "string" ? result.reason : "missing_result";
  const recordId = result?.recordId === expected.recordId ? expected.recordId : null;
  obdOperationJournalRemovalState.outcome = Object.freeze({ status, reason, recordId });
  obdOperationJournalRemovalState.id = expected.recordId;
  if (status === "confirmed" && reason === "record_removed" && recordId === expected.recordId) {
    obdOperationJournalRemovalState.phase = "confirmed";
    obdOperationJournalRemovalState.status = `保存記録を削除しました。 記録ID: ${expected.recordId}`;
    return;
  }
  if (status === "busy" && reason === "record_busy" && recordId === expected.recordId) {
    obdOperationJournalRemovalState.phase = "busy";
    obdOperationJournalRemovalState.status = "この保存記録は別の同一サイト画面で比較中のため削除しませんでした。比較候補を解除後、必要ならもう一度削除してください。";
    return;
  }
  if (status === "unavailable" && reason === "web_locks_unavailable") {
    obdOperationJournalComparisonState.reservationUnavailable = true;
    obdOperationJournalRemovalState.phase = "rejected";
    obdOperationJournalRemovalState.status = "このブラウザでは比較中の記録を保護できないため削除できません。";
    return;
  }
  if (status === "conflict" && recordId === expected.recordId && ["record_not_found", "record_mismatch"].includes(reason)) {
    obdOperationJournalRemovalState.phase = "conflict";
    obdOperationJournalRemovalState.status = "保存記録が選択時の内容と一致しないため削除しませんでした。一覧を更新してください。";
    return;
  }
  if (status === "rejected" && recordId === expected.recordId && [
    "invalid_input", "invalid_record_id", "invalid_session_json", "invalid_created_at",
    "invalid_session_schema", "session_policy_unavailable", "session_policy_rejected",
    "digest_unavailable", "digest_failed"
  ].includes(reason)) {
    obdOperationJournalRemovalState.phase = "rejected";
    obdOperationJournalRemovalState.status = "削除条件を確認できないため削除しませんでした。一覧を更新してください。";
    return;
  }
  obdOperationJournalRemovalState.phase = "unknown";
  obdOperationJournalRemovalState.status = "削除された可能性があります。自動では再試行しません。一覧を更新して確認してください。";
}

function clearMatchingObdOperationJournalLastSave(recordId) {
  if (obdOperationJournalSaveState.id !== recordId) return;
  obdOperationJournalSaveState.phase = "idle";
  obdOperationJournalSaveState.id = null;
  obdOperationJournalSaveState.status = "最後に保管した記録は削除されました。";
  obdOperationJournalSaveState.requiresAck = false;
}

function isObdOperationJournalReservationSupported() {
  return obdOperationJournalComparisonState.reservationUnavailable !== true
    && typeof globalThis.navigator?.locks?.request === "function"
    && typeof window.ObdOperationJournal?.reservePreOperation === "function";
}

function releaseObdOperationJournalComparisonLease(lease) {
  if (!lease || typeof lease.release !== "function") return Promise.resolve();
  if (obdOperationJournalComparisonState.releasingLease === lease && obdOperationJournalComparisonState.releasePromise) {
    return obdOperationJournalComparisonState.releasePromise;
  }
  const releasePromise = Promise.resolve()
    .then(() => lease.release())
    .catch(() => undefined)
    .then(() => undefined);
  obdOperationJournalComparisonState.releasingLease = lease;
  obdOperationJournalComparisonState.releasePromise = releasePromise;
  void releasePromise.then(() => {
    if (obdOperationJournalComparisonState.releasePromise !== releasePromise) return;
    obdOperationJournalComparisonState.releasePromise = null;
    obdOperationJournalComparisonState.releasingLease = null;
    renderObdOperationJournalViewer();
  });
...
function clearObdOperationJournalComparison() {
  obdOperationJournalComparisonState.generation += 1;
  obdDtcClearTargetBindingController.invalidate("journal_association_not_current");
  const lease = obdOperationJournalComparisonState.association?.lease || null;
  obdOperationJournalComparisonState.association = null;
  obdOperationJournalComparisonState.status = "";
  return lease
    ? releaseObdOperationJournalComparisonLease(lease)
    : obdOperationJournalComparisonState.releasePromise || Promise.resolve();
}
...
function getEligibleObdOperationJournalRecord(revision) {
  ...
  if (obdOperationJournalState.selectedRecordOrigin === "list"
    && obdOperationJournalState.recordIds.includes(record.recordId)) return record;
  if (obdOperationJournalState.selectedRecordOrigin === "confirmed-save"
    && obdOperationJournalSaveState.phase === "confirmed"
    && obdOperationJournalSaveState.id === record.recordId
    && !obdOperationJournalSaveState.promise) return record;
  return null;
}

function getObdOperationJournalSaveBlockReason(revision = obdOperationJournalState.revision, { ignoreRemovalBusy = false } = {}) {
  if (!isObdOperationJournalViewerActive(revision)) return "保管画面を開いてから操作してください。";
  if (!ignoreRemovalBusy && obdOperationJournalRemovalState.promise) return "保存記録の削除結果が確定してから保管してください。";
  if (obdOperationJournalState.listing || obdOperationJournalState.loading) return "一覧または記録の読込が完了してから保管してください。";
  if (obdDevSession.pendingWriteOperation || obdDevSession.pendingCommandOperation) return "通信処理が完了してから保管してください。";
...
function getObdOperationJournalComparisonBlockReason(revision, record, sessionRef, { ignoreRemovalBusy = false, ignoreComparisonBusy = false } = {}) {
  if (!record || !isObdOperationJournalViewerActive(revision)
    || obdOperationJournalState.listing || obdOperationJournalState.loading
    || (!ignoreRemovalBusy && Boolean(obdOperationJournalRemovalState.promise))
  ...
  if (!isObdOperationJournalReservationSupported()) return "このブラウザでは保存記録の比較保護を利用できません。";
  if (!ignoreComparisonBusy && (obdOperationJournalComparisonState.promise || obdOperationJournalComparisonState.releasePromise)) {
    return "比較候補の保護処理が完了してから操作してください。";
  }
  if (!ignoreComparisonBusy && obdOperationJournalComparisonState.association) return "比較候補を解除してから新しい照合をしてください。";
  return getObdOperationJournalSaveBlockReason(revision, { ignoreRemovalBusy });
}

function getObdOperationJournalRemovalBlockReason(revision, record, { ignoreOwnBusy = false } = {}) {
  if (!record || !isObdOperationJournalViewerActive(revision)
    || obdOperationJournalState.listing || obdOperationJournalState.loading
    || Boolean(obdOperationJournalSaveState.promise)
    || (!ignoreOwnBusy && Boolean(obdOperationJournalRemovalState.promise))
    || getEligibleObdOperationJournalRecord(revision) !== record) return "検証済みの記録を選択してください。";
  if (!isObdOperationJournalReservationSupported()) return "このブラウザでは比較中の記録を保護できないため削除できません。";
  if (obdOperationJournalComparisonState.promise || obdOperationJournalComparisonState.releasePromise) {
    return "比較候補の保護処理が完了してから削除してください。";
  }
  if (obdDevSession.pendingWriteOperation || obdDevSession.pendingCommandOperation
    || obdDevSession.readInProgress || obdDevSession.coreScanInProgress || obdDevSession.initializing
    || obdSerialDisconnectOperation || obdSerialConnectPending || obdBridgeOperation) {
    return "通信処理が完了してから削除してください。";
  }
  // Deliberately avoid the validating accessor here: it may clear a candidate as a side effect.
  if (obdOperationJournalComparisonState.association?.record === record) return "比較候補を解除してから削除してください。";
  if (["unknown", "conflict", "rejected"].includes(obdOperationJournalRemovalState.phase)
    && obdOperationJournalRemovalState.id === record.recordId) {
    return "削除結果を一覧更新で確認してから次の操作をしてください。";
  }
  return "";
}
...
function getObdOperationJournalComparisonAssociation() {
  ...
  const blockReason = getObdOperationJournalComparisonBlockReason(association.revision, association.record, association.sessionRef, { ignoreRemovalBusy: true, ignoreComparisonBusy: true });
  if (blockReason || association.record.sessionJson !== association.sessionJson
    || association.record.createdAt !== association.createdAt
    || association.lease?.recordId !== association.record.recordId || association.lease?.isHeld?.() !== true
  ...
  try {
    if (buildCurrentObdOperationJournalSaveSnapshot(association.sessionRef, association.exportedAt) !== association.sessionJson
      || getObdOperationJournalComparisonBlockReason(association.revision, association.record, association.sessionRef, { ignoreRemovalBusy: true, ignoreComparisonBusy: true })
      || !isSafeObdOperationJournalComparisonController(association.controller, association.record.recordId)) {
      clearObdOperationJournalComparison();
      return null;
    }
...
async function compareCurrentObdReadoutToOperationJournalRecord() {
  ...
  const generation = ++obdOperationJournalComparisonState.generation;
  const revision = obdOperationJournalState.revision;
  const selectionGeneration = obdOperationJournalState.selectionGeneration;
  const record = getEligibleObdOperationJournalRecord(revision);
  const sessionRef = obdDevSession.lastSession;
  let operation;
  operation = (async () => {
    const blockReason = getObdOperationJournalComparisonBlockReason(revision, record, sessionRef, { ignoreComparisonBusy: true });
    if (blockReason) {
      obdOperationJournalState.status = blockReason;
      obdOperationJournalState.error = blockReason !== "このブラウザでは保存記録の比較保護を利用できません。";
      renderObdOperationJournalViewer();
      return false;
    }
    const expected = Object.freeze({ recordId: record.recordId, sessionJson: record.sessionJson, createdAt: record.createdAt });
    const exportedAt = getObdOperationJournalRecordExportedAt(expected.sessionJson);
    if (!exportedAt) {
    ...
    try {
      if (buildCurrentObdOperationJournalSaveSnapshot(sessionRef, exportedAt) !== expected.sessionJson) {
    ...
    obdOperationJournalState.status = "保存記録を確認して比較を保護しています。";
    obdOperationJournalState.error = false;
    renderObdOperationJournalViewer();
    let reservation;
    try {
      reservation = await window.ObdOperationJournal.reservePreOperation(expected);
    } catch (_error) {
      reservation = null;
    }
    const lease = reservation?.lease;
    const current = obdOperationJournalComparisonState.promise === operation
      && obdOperationJournalComparisonState.generation === generation
      && isObdOperationJournalViewerActive(revision)
      && obdOperationJournalState.selectionGeneration === selectionGeneration
      && obdOperationJournalState.selectedRecord === record
      && record.recordId === expected.recordId && record.sessionJson === expected.sessionJson && record.createdAt === expected.createdAt
      && obdDevSession.lastSession === sessionRef;
    let currentSessionMatches = false;
    try { currentSessionMatches = buildCurrentObdOperationJournalSaveSnapshot(sessionRef, exportedAt) === expected.sessionJson; } catch (_error) { /* Stale input is rejected below. */ }
    const reserved = reservation?.status === "reserved" && reservation.reason === "record_reserved"
      && reservation.recordId === expected.recordId && reservation.execution?.wouldTransmit === false
      && reservation.execution?.canExecute === false && reservation.execution?.retryAllowed === false
      && lease?.recordId === expected.recordId && typeof lease?.isHeld === "function" && lease.isHeld() === true;
    if (!current || !currentSessionMatches || !reserved
      || getObdOperationJournalComparisonBlockReason(revision, record, sessionRef, { ignoreComparisonBusy: true })) {
      if (lease) void releaseObdOperationJournalComparisonLease(lease);
      if (!current) return false;
      if (reservation?.status === "busy" && reservation.reason === "record_busy" && reservation.recordId === expected.recordId) {
        obdOperationJournalState.status = "この保存記録は別の同一サイト画面で使用中です。比較候補を解除後、手動で再試行してください。";
        obdOperationJournalState.error = false;
      } else if (reservation?.status === "unavailable" && reservation.reason === "web_locks_unavailable") {
        obdOperationJournalComparisonState.reservationUnavailable = true;
        obdOperationJournalState.status = "このブラウザでは保存記録の比較保護を利用できません。";
        obdOperationJournalState.error = false;
      } else {
        obdOperationJournalState.status = "照合条件が変わったため、実行しませんでした。";
        obdOperationJournalState.error = true;
      }
      renderObdOperationJournalViewer();
      return false;
    ...
    let controller;
    try {
      controller = window.ObdReadOnly?.createGenericObdDtcClearController?.({ target: null, preOperationSessionId: expected.recordId, evidence: {} });
    } catch (_error) {
      controller = null;
    }
    if (!isSafeObdOperationJournalComparisonController(controller, expected.recordId)) {
      void releaseObdOperationJournalComparisonLease(lease);
    ...
    const association = Object.freeze({
      ...
      record,
      sessionRef,
      sessionJson: expected.sessionJson,
    ...
    obdOperationJournalComparisonState.association = association;
    try {
    ...
    obdOperationJournalComparisonState.status = `照合時点の読取内容が一致。車両同一性・適合・安全条件・消去許可は未確認。 記録ID: ${expected.recordId}`;
    obdOperationJournalState.status = "";
    obdOperationJournalState.error = false;
    renderObdOperationJournalViewer();
    return true;
  })().finally(() => {
    if (obdOperationJournalComparisonState.promise === operation) obdOperationJournalComparisonState.promise = null;
    renderObdOperationJournalViewer();
  });
  obdOperationJournalComparisonState.promise = operation;
  renderObdOperationJournalViewer();
  return operation;
}

function renderObdOperationJournalSaveInfo(active) {
  if (!obdOperationJournalSaveCurrent || !obdOperationJournalSaveStatus || !obdOperationJournalSaveAck || !obdOperationJournalSaveAckInput) return;
  if (!active) {
    obdOperationJournalSaveCurrent.disabled = true;
    obdOperationJournalSaveCurrent.title = "保管画面を開いてから操作してください。";
    obdOperationJournalSaveStatus.textContent = "";
    obdOperationJournalSaveStatus.classList.remove("error");
    obdOperationJournalSaveAck.hidden = true;
    obdOperationJournalSaveAckInput.checked = false;
    return;
  }
  const blockReason = getObdOperationJournalSaveBlockReason(obdOperationJournalState.revision);
  const waitingForAck = obdOperationJournalSaveState.requiresAck === true;
  obdOperationJournalSaveCurrent.disabled = Boolean(obdOperationJournalSaveState.promise) || Boolean(blockReason) || (waitingForAck && !obdOperationJournalSaveAckInput.checked);
  obdOperationJournalSaveCurrent.title = blockReason || (waitingForAck && !obdOperationJournalSaveAckInput.checked
    ? "前回の保存結果が不明なため、確認してから新しいIDで開始してください。"
    : "現在の読取結果をこのブラウザに保管");
  const saveId = ["confirmed", "unknown"].includes(obdOperationJournalSaveState.phase) && obdOperationJournalSaveState.id
    ? ` 記録ID: ${obdOperationJournalSaveState.id}`
    : "";
  obdOperationJournalSaveStatus.textContent = `${obdOperationJournalSaveState.status}${saveId}`;
  obdOperationJournalSaveStatus.classList.toggle("error", ["unknown", "rejected"].includes(obdOperationJournalSaveState.phase));
  obdOperationJournalSaveAck.hidden = !waitingForAck;
}

function buildCurrentObdOperationJournalSaveSnapshot(sessionRef, exportedAt) {
  const buildExport = window.ObdReadOnly?.buildBridgeSessionExportPayload;
  const policy = window.ObdReadOnly?.getDiagnosticSessionJsonPolicy;
  if (typeof buildExport !== "function" || typeof policy !== "function") throw new Error("journal_export_unavailable");
  const payload = buildExport({ ...sessionRef, exportedAt });
  if (!payload || payload.schema_version !== "bridge_session_export_v1" || !payload.session || typeof payload.session !== "object" || Array.isArray(payload.session)) {
    throw new Error("journal_export_invalid");
  }
  ...
  if (bytes.byteLength > 4000000 || policyResult?.accepted !== true || policyResult.kind !== "session") throw new Error("journal_export_rejected");
  return sessionJson;
}

function createObdOperationJournalRecordId() {
  const random = new Uint8Array(16);
  if (!globalThis.crypto || typeof globalThis.crypto.getRandomValues !== "function") throw new Error("journal_random_unavailable");
  globalThis.crypto.getRandomValues(random);
  return `journal-${Array.from(random, (byte) => byte.toString(16).padStart(2, "0")).join("")}`;
}

async function saveCurrentObdReadoutToOperationJournal() {
  if (obdOperationJournalSaveState.promise) return false;
  let operation;
  operation = (async () => {
    // Yield after publishing the sentinel so synthetic and user reentrancy cannot duplicate confirmation or writes.
    await Promise.resolve();
    try {
    const revision = obdOperationJournalState.revision;
    const blockReason = getObdOperationJournalSaveBlockReason(revision);
    if (blockReason) {
      obdOperationJournalSaveState.status = blockReason;
      renderObdOperationJournalViewer();
      return false;
    }
    if (obdOperationJournalSaveState.requiresAck && !obdOperationJournalSaveAckInput?.checked) {
      obdOperationJournalSaveState.status = "前回の保存結果が不明です。確認してから新しいIDで開始してください。";
      renderObdOperationJournalViewer();
      return false;
    }
    const sessionRef = obdDevSession.lastSession;
    ...
    let sessionJson;
    try {
      sessionJson = buildCurrentObdOperationJournalSaveSnapshot(sessionRef, exportedAt);
    } catch (_error) {
      obdOperationJournalSaveState.status = "保管用JSONを確認できませんでした。読取結果は変更していません。";
      renderObdOperationJournalViewer();
      return false;
    }
    clearObdOperationJournalComparison();
    ...
    let confirmed;
    try {
      confirmed = confirm("現在の読取結果を端末内に保管します。車両通信、DTC消去、消去の許可は行いません。読取結果は変更しません。個人・車両情報を含む可能性があるため共有端末では注意してください。続行しますか？");
    } catch (_error) {
      obdOperationJournalSaveState.phase = "idle";
      obdOperationJournalSaveState.status = "保管の確認を開始できませんでした。読取結果は変更していません。";
      renderObdOperationJournalViewer();
      return false;
    }
    if (!confirmed) {
      obdOperationJournalSaveState.phase = "idle";
      obdOperationJournalSaveState.status = "保管を取り消しました。読取結果は変更していません。";
      renderObdOperationJournalViewer();
      return false;
    }
    const afterConfirmReason = getObdOperationJournalSaveBlockReason(revision);
    if (afterConfirmReason || obdDevSession.lastSession !== sessionRef) {
      obdOperationJournalSaveState.phase = "idle";
      obdOperationJournalSaveState.status = afterConfirmReason || "読取結果が更新されたため、保管を開始しませんでした。";
      renderObdOperationJournalViewer();
      return false;
    }
    let rebuiltJson;
    try {
      rebuiltJson = buildCurrentObdOperationJournalSaveSnapshot(sessionRef, exportedAt);
    } catch (_error) {
      obdOperationJournalSaveState.phase = "idle";
      obdOperationJournalSaveState.status = "確認後に保管用JSONを再確認できませんでした。読取結果は変更していません。";
      renderObdOperationJournalViewer();
      return false;
    }
    if (rebuiltJson !== sessionJson || getObdOperationJournalSaveBlockReason(revision) || obdDevSession.lastSession !== sessionRef) {
      obdOperationJournalSaveState.phase = "idle";
      obdOperationJournalSaveState.status = "読取結果または保管条件が変わったため、保管を開始しませんでした。";
      renderObdOperationJournalViewer();
      return false;
    }
    let recordId;
    try {
      recordId = createObdOperationJournalRecordId();
    } catch (_error) {
      obdOperationJournalSaveState.phase = "idle";
      obdOperationJournalSaveState.status = "安全な保管IDを作成できないため、保管を開始しませんでした。";
      renderObdOperationJournalViewer();
      return false;
    }
    obdOperationJournalSaveState.requiresAck = false;
    if (obdOperationJournalSaveAckInput) obdOperationJournalSaveAckInput.checked = false;
    obdOperationJournalSaveState.id = recordId;
    obdOperationJournalSaveState.phase = "pending";
    obdOperationJournalSaveState.status = "現在の読取結果を保管しています。";
    renderObdOperationJournalViewer();
    try {
      const result = await window.ObdOperationJournal?.savePreOperation?.({ recordId, sessionJson });
      if (result?.status === "confirmed" && result.reason === "record_confirmed" && result.recordId === recordId) {
        obdOperationJournalSaveState.phase = "confirmed";
        obdOperationJournalSaveState.id = result.recordId;
        obdOperationJournalSaveState.status = "端末内に保管しました。";
        return true;
      }
      const knownConflict = result?.status === "conflict" && result.recordId === recordId && result.reason === "record_conflict";
      const knownRejected = result?.status === "rejected" && result.recordId === recordId && [
        "invalid_input", "invalid_record_id", "invalid_session_json", "invalid_session_schema",
        "session_policy_unavailable", "session_policy_rejected", "digest_failed", "digest_unavailable"
      ].includes(result.reason);
      if (knownConflict || knownRejected) {
        obdOperationJournalSaveState.phase = "rejected";
        obdOperationJournalSaveState.status = "保管を確定できませんでした。自動では再試行しません。";
        return false;
      }
      obdOperationJournalSaveState.phase = "unknown";
      obdOperationJournalSaveState.status = "保存された可能性があります。";
      obdOperationJournalSaveState.requiresAck = true;
      return false;
    } catch (_error) {
      obdOperationJournalSaveState.phase = "unknown";
      obdOperationJournalSaveState.status = "保存された可能性があります。";
      ...
      if (obdOperationJournalSaveState.promise === operation) obdOperationJournalSaveState.promise = null;
      renderObdOperationJournalViewer();
    }
  })();
  obdOperationJournalSaveState.promise = operation;
  renderObdOperationJournalViewer();
  return operation;
}
...
function renderObdOperationJournalViewer() {
  ...
  renderObdOperationJournalSaveInfo(active);
  if (obdOperationJournalRemoveStatus) {
    obdOperationJournalRemoveStatus.textContent = active ? obdOperationJournalRemovalState.status : "";
    obdOperationJournalRemoveStatus.classList.toggle("error", active && ["unknown", "conflict", "rejected"].includes(obdOperationJournalRemovalState.phase));
  }
  ...
  const removalBusy = Boolean(obdOperationJournalRemovalState.promise);
  obdOperationJournalRefresh.disabled = !active || obdOperationJournalState.listing || removalBusy;
  obdOperationJournalPrevious.disabled = !active || obdOperationJournalState.listing || obdOperationJournalState.loading || removalBusy || obdOperationJournalState.pageIndex === 0;
  obdOperationJournalNext.disabled = !active || obdOperationJournalState.listing || obdOperationJournalState.loading || removalBusy || page?.hasMore !== true || !page.nextAfterRecordId;
  if (obdOperationJournalOpenConfirmed) {
    const confirmedId = obdOperationJournalSaveState.id;
    obdOperationJournalOpenConfirmed.disabled = !active || obdOperationJournalState.listing || obdOperationJournalState.loading
      || Boolean(obdOperationJournalSaveState.promise) || removalBusy || obdOperationJournalSaveState.phase !== "confirmed"
  ...
  if (active && obdOperationJournalState.recordIds.length) {
    obdOperationJournalState.recordIds.forEach((recordId) => {
      ...
      button.disabled = obdOperationJournalState.listing || obdOperationJournalState.loading || removalBusy;
  ...
  remove.type = "button";
  remove.className = "small-danger-button obd-operation-journal-remove";
  remove.textContent = "選択した記録を削除";
  const removeBlockReason = getObdOperationJournalRemovalBlockReason(obdOperationJournalState.revision, record);
  remove.disabled = Boolean(removeBlockReason);
  ...
  if (comparisonEvidence) {
    ...
    obdOperationJournalRecord.append(metadata, comparison, preparation, jsonDetails, remove);
  } else {
    obdOperationJournalRecord.append(metadata, jsonDetails, remove);
  }
}

async function removeSelectedObdOperationJournalRecord() {
  if (obdOperationJournalRemovalState.promise) return false;
  let operation;
  operation = (async () => {
    await Promise.resolve();
    const revision = obdOperationJournalState.revision;
    const generation = obdOperationJournalState.selectionGeneration;
    const record = getEligibleObdOperationJournalRecord(revision);
    const blockReason = getObdOperationJournalRemovalBlockReason(revision, record, { ignoreOwnBusy: true });
    if (blockReason) {
      obdOperationJournalRemovalState.status = blockReason;
      renderObdOperationJournalViewer();
      return false;
    }
    const expected = Object.freeze({ recordId: record.recordId, sessionJson: record.sessionJson, createdAt: record.createdAt });
    obdOperationJournalRemovalState.phase = "confirming";
    obdOperationJournalRemovalState.id = record.recordId;
    obdOperationJournalRemovalState.status = "削除の確認待ちです。";
    renderObdOperationJournalViewer();
    let confirmed;
    try {
      confirmed = confirm(`保存記録 ${record.recordId} を端末内から削除します。この操作は元に戻せません。現在の診断結果は変更せず、車両通信も行いません。続行しますか？`);
    } catch (_error) {
      obdOperationJournalRemovalState.phase = "rejected";
      obdOperationJournalRemovalState.status = "削除の確認を開始できませんでした。記録は削除していません。";
      return false;
    }
    if (!confirmed) {
      obdOperationJournalRemovalState.phase = "idle";
      obdOperationJournalRemovalState.id = null;
      obdOperationJournalRemovalState.status = "削除を取り消しました。記録と現在の診断結果は変更していません。";
      return false;
    }
    const afterConfirmReason = getObdOperationJournalRemovalBlockReason(revision, record, { ignoreOwnBusy: true });
    if (afterConfirmReason || obdOperationJournalState.selectionGeneration !== generation
      || obdOperationJournalState.selectedRecord !== record || record.recordId !== expected.recordId
      || record.sessionJson !== expected.sessionJson || record.createdAt !== expected.createdAt) {
      obdOperationJournalRemovalState.phase = "rejected";
      obdOperationJournalRemovalState.status = afterConfirmReason || "選択した記録が変わったため、削除を開始しませんでした。";
      return false;
    }
    obdOperationJournalRemovalState.phase = "pending";
    obdOperationJournalRemovalState.status = "選択した保存記録を削除しています。";
    renderObdOperationJournalViewer();
    try {
      const result = await window.ObdOperationJournal?.removePreOperation?.(expected);
      const confirmedRemoval = result?.status === "confirmed" && result.reason === "record_removed" && result.recordId === expected.recordId;
      if (confirmedRemoval) clearMatchingObdOperationJournalLastSave(expected.recordId);
      const resultStillCurrent = isObdOperationJournalViewerActive(revision)
        && obdOperationJournalState.selectionGeneration === generation
        && obdOperationJournalState.selectedRecord === record
        && record.recordId === expected.recordId
        && record.sessionJson === expected.sessionJson
        && record.createdAt === expected.createdAt;
      if (!resultStillCurrent) {
        rememberObdOperationJournalRemovalOutcome(result, expected);
        return false;
      }
      if (confirmedRemoval) {
        obdOperationJournalRemovalState.phase = "confirmed";
        ...
        obdOperationJournalState.recordIds = [];
        setObdOperationJournalSelectedRecord(null, null);
        obdOperationJournalState.status = "選択した保存記録の削除を確認しました。現在の診断結果は変更していません。";
        obdOperationJournalState.error = false;
        return true;
      }
      if (result?.status === "busy" && result.reason === "record_busy" && result.recordId === expected.recordId) {
        obdOperationJournalRemovalState.phase = "busy";
        obdOperationJournalRemovalState.status = "この保存記録は別の同一サイト画面で比較中のため削除しませんでした。比較候補を解除後、必要ならもう一度削除してください。";
        return false;
      }
      if (result?.status === "unavailable" && result.reason === "web_locks_unavailable") {
        obdOperationJournalComparisonState.reservationUnavailable = true;
        obdOperationJournalRemovalState.phase = "rejected";
        obdOperationJournalRemovalState.status = "このブラウザでは比較中の記録を保護できないため削除できません。";
        return false;
      }
      if (result?.status === "conflict" && result.recordId === expected.recordId
        && ["record_not_found", "record_mismatch"].includes(result.reason)) {
        obdOperationJournalRemovalState.phase = "conflict";
        obdOperationJournalRemovalState.status = "保存記録が選択時の内容と一致しないため削除しませんでした。一覧を更新してください。";
        return false;
      }
      const knownRejected = result?.status === "rejected" && result.recordId === expected.recordId && [
        "invalid_input", "invalid_record_id", "invalid_session_json", "invalid_created_at",
        "invalid_session_schema", "session_policy_unavailable", "session_policy_rejected",
        "digest_unavailable", "digest_failed"
      ].includes(result.reason);
      ...
      if (!isObdOperationJournalViewerActive(revision)
        || obdOperationJournalState.selectionGeneration !== generation
        || obdOperationJournalState.selectedRecord !== record) {
        rememberObdOperationJournalRemovalOutcome({ status: "indeterminate", reason: "operation_exception", recordId: expected.recordId }, expected);
        return false;
      }
      obdOperationJournalRemovalState.phase = "unknown";
      obdOperationJournalRemovalState.status = "削除された可能性があります。自動では再試行しません。一覧を更新して確認してください。";
    ...
    if (obdOperationJournalRemovalState.promise === operation) obdOperationJournalRemovalState.promise = null;
    renderObdOperationJournalViewer();
  });
  obdOperationJournalRemovalState.promise = operation;
  renderObdOperationJournalViewer();
  return operation;
}
...
async function listObdOperationJournalRecords({ reset = false, pageIndex = 0 } = {}) {
  if (!isObdOperationJournalViewerActive(obdOperationJournalState.revision) || obdOperationJournalRemovalState.promise) return;
...
async function loadObdOperationJournalRecord(recordId) {
  if (!isObdOperationJournalViewerActive(obdOperationJournalState.revision)
    || obdOperationJournalRemovalState.promise
  ...
  try {
    ...
    setObdOperationJournalSelectedRecord(result.record, "list");
...
async function openConfirmedObdOperationJournalRecord() {
  const recordId = obdOperationJournalSaveState.id;
  if (!isObdOperationJournalViewerActive(obdOperationJournalState.revision)
    || obdOperationJournalState.listing || obdOperationJournalState.loading
    || obdOperationJournalSaveState.promise || obdOperationJournalRemovalState.promise || obdOperationJournalSaveState.phase !== "confirmed"
  ...
  resetObdOperationJournalRemovalOutcome();
  setObdOperationJournalSelectedRecord(null, null);
  obdOperationJournalState.status = "保管した記録を検証して読み込んでいます。";
  obdOperationJournalState.error = false;
  renderObdOperationJournalViewer();
  try {
    const result = await window.ObdOperationJournal?.loadPreOperation?.({ recordId });
    if (!isObdOperationJournalViewerActive(revision)
      ...
      || obdOperationJournalSaveState.id !== recordId) return;
    if (result?.status !== "loaded" || result.reason !== "valid_record_recovered" || !result.record
      || result.record.recordId !== recordId || typeof result.record.sessionJson !== "string") throw new Error("load_unavailable");
    setObdOperationJournalSelectedRecord(result.record, "confirmed-save");
    obdOperationJournalState.status = "保管した記録を検証して読み込みました。診断結果には反映していません。";
  } catch (_error) {
    if (!isObdOperationJournalViewerActive(revision)) return;
    setObdOperationJournalSelectedRecord(null, null);
    obdOperationJournalState.status = "保管した記録を検証して読み込めませんでした。";
    obdOperationJournalState.error = true;
  } finally {
    if (!isObdOperationJournalViewerActive(revision)) return;
    obdOperationJournalState.loading = false;
...
function navigateToObdReadoutControl(targetButton) {
  setObdUiMode("details");
  const target = !obdAccessUnlocked ? obdAccessPasswordInput
    : !obdDevModeUnlocked ? obdDevPasswordInput
    : targetButton || obdDevControls || obdDevSessionSummary;
  revealObdControlGroup(target);
  target?.scrollIntoView({ behavior: "smooth", block: "center" });
  const focusTarget = target === obdDevControls ? target.querySelector("summary")
    : target?.disabled ? target.closest("details")?.querySelector("summary") : target;
  focusTarget?.focus();
}
...
function formatObdSessionSourceLabel(source, fallback = NO_DATA) {
  return {
    j2534_development_read: "J2534開発検証データ（実車読取ではありません）",
...
function renderObdBridgeSessionDetails(session = null) {
  ...
  if (timelineChartRows.length) {
    ...
    card.id = "obdSessionDetailLiveTimeline";
    const heading = document.createElement("strong");
    ...
    timelineChartRows.forEach((row) => {
      ...
      position.className = "obd-timeline-position";
      const positionLabel = document.createElement("span");
      positionLabel.textContent = "記録位置";
      const slider = document.createElement("input");
      slider.type = "range";
      slider.min = "0";
      slider.max = String(row.points.length - 1);
      slider.step = "1";
...
function formatVehicleApplicabilityEcuObservationReviewPlan(plan, fallback = NO_DATA) {
  if (!plan || typeof plan !== "object") return fallback;
  if (plan.reviewRequired !== true && plan.review_required !== true) return "再確認不要";
  const targetCount = Number(plan.targetObservationCount ?? plan.target_observation_count ?? 0);
  const applicabilityCount = Number(plan.applicabilityReviewObservationCount ?? plan.applicability_review_observation_count ?? 0);
  const addresses = Array.isArray(plan.targetAddressDescriptors)
    ? plan.targetAddressDescriptors
    : Array.isArray(plan.target_address_descriptors) ? plan.target_address_descriptors : [];
  const parts = [];
  if (targetCount > 0) parts.push(`ECU情報再確認 ${targetCount}件`);
  if (addresses.length) parts.push(addresses.slice(0, 3).join(","));
...
function renderObdDiagnosticFlowPanel(session = null) {
  ...
  const vehicleApplicabilityEcuObservationReviewPlan = importedSessionComparisonSummary?.vehicleApplicabilityEcuObservationReviewPlan
    || importedSessionComparisonSummary?.vehicle_applicability_ecu_observation_review_plan
    || null;
  const vehicleApplicabilityEcuObservationReviewLabel = formatVehicleApplicabilityEcuObservationReviewPlan(vehicleApplicabilityEcuObservationReviewPlan, NO_DATA);
  ...
  addObdDiagnosticFlowMetric(grid, "ECU再確認", vehicleApplicabilityEcuObservationReviewLabel, vehicleApplicabilityEcuObservationReviewPlan?.reviewRequired === true ? "pending" : "");
  ...
  if (nextReadoutId) {
    ...
    button.addEventListener("click", () => {
      navigateToObdReadoutControl(targetButton);
    });
...
function renderObdSimpleResultSummary(session = null) {
  ...
  const dtcStatusBreakdown = formatObdBridgeDtcStatusSummary(dtcSnapshot?.dtcs || dtcSnapshot?.codes || [])
    .replace(/^ 内訳: /, "")
    .replace(/。$/, "");
  const dtcReadoutStatusSummary = formatObdDtcReadoutStatusSummary(
    dtcSnapshot?.dtcStatusSummary || dtcSnapshot?.dtc_status_summary || coreStatus?.dtcStatusSummary || coreStatus?.dtc_status_summary,
    ""
  );
  ...
  const resolveReadoutState = (readoutId) => {
    ...
    if (coverageItem?.status === "empty") return { label: readoutId === "dtc_snapshot" ? "コード0件" : "応答0件", tone: "empty" };
...
function renderObdDeveloperSessionSummary(session = null) {
  ...
  values.splice(6, 0, ["ECU再確認", vehicleApplicabilityEcuObservationReviewLabel]);
...
function analyzeObdScannerImport(options = {}) {
  ...
  const vehicleApplicabilityEcuObservationReviewPlan = importedSessionComparisonSummary?.vehicleApplicabilityEcuObservationReviewPlan
    || importedSessionComparisonSummary?.vehicle_applicability_ecu_observation_review_plan
    || null;
  const vehicleApplicabilityEcuObservationReviewNote = formatVehicleApplicabilityEcuObservationReviewPlan(vehicleApplicabilityEcuObservationReviewPlan, "");
  if (vehicleApplicabilityEcuObservationReviewNote) {
    notes.push(`ECU再確認 ${vehicleApplicabilityEcuObservationReviewNote}`);
  }
...
function invalidateObdScannerImport(preserveInput = null) {
  ...
  } catch (_) { /* Late callbacks remain invalidated even if abort fails. */ }
  renderObdSessionExportControls();
}
    ...
    try {
      await Promise.race([
        ...
        new Promise((_, reject) => {
          timeout = setTimeout(() => {
            timedOut = true;
            reject(new Error("clipboard_write_timeout"));
          }, 30000);
        })
      ]);
      return;
    } catch (error) {
      // A pending OS write cannot be cancelled. Do not issue a second write on timeout.
...
function renderCases() {
  ...
  // Normalize only the search projection; never rewrite saved evidence.
  const keyword = caseSearch.value.normalize("NFKC").trim().toLowerCase();
  const terms = keyword.split(/\s+/).filter(Boolean);
  const filtered = savedCases.filter((item) => {
    const target = [item.id, item.maker, item.model, item.symptom, item.obdCode, item.finalCause, item.confirmedFacts, item.work, item.replacedParts, item.memo,
      item.year, item.engine, item.mileage, item.technician, item.measurements].join(" ").normalize("NFKC").toLowerCase();
    return terms.every((term) => target.includes(term));
  });

  const emptyText = caseStorageReadError ? "保存事例を読み込めていません。"
    : savedCases.length ? "検索条件に一致する事例はありません。検索語を減らすか変更してください。"
    : "保存済み事例はまだありません。";
  renderCaseCards(caseList, filtered, emptyText);
...
function clearCaseExportStatus() {
...
function exportCasesJson() {
  ...
  try {
    ...
    document.body.appendChild(link);
    link.click();
    if (status) status.textContent = `整備事例${backup.records.length}件のJSON保存を開始しました。保存完了はブラウザーのダウンロード一覧で確認してください。`;
    return true;
  } catch (_) {
    if (status) status.textContent = "JSONバックアップの保存を開始できませんでした。端末内の整備事例は変更していません。ブラウザーの保存設定・空き容量を確認し、改めて保存してください。";
    return false;
  } finally {
    link?.remove();
    if (objectUrl) setTimeout(() => URL.revokeObjectURL(objectUrl), 0);
...
function importCasesJson(event) {
  ...
  reader.onload = () => {
    ...
    try {
      ...
      let invalid = 0;
      const nextCases = [...savedCases];
      // Keep the existing key and strict ID matching, but index once per import.
      const knownKeys = new Set(nextCases.map(duplicateKey));
      ...
      records.forEach((item) => {
        ...
        const key = duplicateKey(record);
        if (knownKeys.has(key) || knownIds.has(record.id)) {
          skipped += 1;
          return;
        }
