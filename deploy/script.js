...
const OBD_INTERFACE_PROGRESS = Object.freeze({
  ...
  uds_canfd: Object.freeze({
    progressPercent: 85,
    currentBasis: "read-only DID / ECU情報、拡張DTC、NRC、ISO-TPログ取込、DID要求マニフェスト、bridge ECU範囲一致、transport resultからcompletion manifestへのfail-closed変換、J2534隔離worker出力、x86/x64 native transport fixtureの7状態とbounded supervisor監督、attempt ID付き非実行ライフサイクルまで実装済み。実transportアダプターは未実装。",
    nextBuild: "readout attempt controllerでselected device、attempt ID、ECU、DID、operation nonceを発行時からcompletionまで束縛する。",
...
const OBD_CORE_PROGRESS_SNAPSHOT = Object.freeze({
  ...
  recentMilestone: "J2534 native UDS fixtureをbounded supervisorで監督",
  scopeNote: "自動検証件数は実車確認済み車種数や完成率ではありません"
});
const APP_VERSION = "3.13.370";
